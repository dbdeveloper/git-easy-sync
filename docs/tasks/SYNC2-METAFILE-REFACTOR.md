...
`{ mtime, size }` **недостатньо**. Це рівно та конструкція, яку зняли з коду
(`void recorded;` у трьох `enforce*`): при апгрейді плагіна змінюється сама КОНСТАНТА
блоку, а файл на диску не рухається → `mtime`+`size` збігаються → перевірка каже
«свіжо» → нові канонічні рядки **ніколи не доїжджають**.

Тому запис — **три** поля:

```
{ mtime, size, blockFingerprint }   # напр. sha1 канонічного блоку, який ми ТОДІ записали
```

і short-circuit спрацьовує **лише коли збіглися всі три**. Розійшовся будь-який (у т.ч.
`blockFingerprint`, бо ми оновили плагін) → повний `read + splice + compare`, як зараз.
Це зберігає наявний фікс і водночас повертає дешевий шлях.

`blockFingerprint` рахується від **бажаного** блоку для ПОТОЧНОГО стану — отже перемикач
`syncConfigDir` міняє його автоматично, і kill-switch-пара (DOT-FILES §3.1.1) не потребує
окремого поля «чи вона зараз має бути».
