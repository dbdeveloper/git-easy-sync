...
**Що дає третє поле.** Записуємо ще й той блок, який ми **тоді** вставили:

```
{ mtime, size, lastWrittenBlock }
```

На перевірці порівнюємо його з блоком, який **зараз хочемо** (тобто з поточною константою
для поточного стану):

```
плагін 1.0:  lastWrittenBlock == бажаний(A)  → усі три збіглись → пропускаємо      ✅ дешево
плагін 2.0:  lastWrittenBlock(A) != бажаний(B) → розійшлось     → read+splice+write ✅ блок доїхав
```
...
⚠️ **Зберігаємо ТЕКСТ блоку, а не його хеш** — і це не марнотратство, а друга функція
поля. Хеш відповів би тільки на питання «збіглось / не збіглось». Текст додатково дає
змогу **знайти й вирізати старий блок за буквальним збігом**, коли маркери в файлі
пошкоджені — див. DOT-FILES §3.1.3 (порятунок при втраченому `END`). Ціна — кількасот
байт на файл; блок і так короткий, а для чужих `plugins/*/.gitignore` це взагалі два
рядки kill-switch.
