...
**Що дає третє поле.** Записуємо ще й відбиток (напр. sha1) того блоку, який ми **тоді**
вставили:

```
{ mtime, size, blockFingerprint }
```

На перевірці рахуємо відбиток блоку, який **зараз хочемо** (тобто поточної константи):

```
плагін 1.0:  sha1(A) == sha1(A)  → усі три збіглись → пропускаємо   ✅ дешево
плагін 2.0:  sha1(B) != sha1(A)  → розійшлось       → read+splice+write  ✅ блок доїхав
```

Тобто відбиток відповідає на ДРУГЕ питання: «чи те, що ми колись записали, — це все ще
