...
**Onload-доганяння — НАЙПЕРША перевірка старту (доповнення власника, 2026-08-30):**
*«перевірка цієї мітки має бути найпершою — ще до усіх інших способів відновлення —
бо вона відміняє всі решта відновлень»*. Блок стоїть на самому початку `onload`,
ПЕРЕД self-update bootloader-ом, atomic-write sweep-ом, recovery конфліктів і
autosave: стан, який ці механізми «відновлювали б», засуджений підтвердженим reset-ом.
Хвіст доганяється: знос `.runtime/` + settings = DEFAULTS + видалити мітку. Logger
на цей момент ще не існує — помилки в console + Notice; невдале доганяння лишає
мітку → наступний старт повторить. Latch окремо чистити не треба — він конструюється
пізніше і сідає з уже видаленого дискового маркера. Недочищений reset без мітки
