...
### 3.3 Open-guard write-set (спільний патерн)
- History: write-set = **`{currentFile}`** (база=історія, нікуди не пишеться). Наслідок: не
  можна одночасно тримати History `note.md` І конфлікт `note.md` (R-B Notice) — узгоджений
  наслідок правила.
- Deleted: write-set = **`{originalPath}`** (sibling=trash нікуди не пишеться).
