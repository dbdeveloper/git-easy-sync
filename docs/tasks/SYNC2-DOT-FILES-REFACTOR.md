...
> §6 (окремий файл у `.runtime/`, запис = `{mtime, size, lastWrittenBlock}`). Тут — сам
> прохід.
...
для кожного файлу з множини:
    ...
    збіглися mtime + size + lastWrittenBlock?  → нічого не робимо
...
⚠️ `lastWrittenBlock` у порівнянні **обов'язковий** — без нього апгрейд плагіна, що
...
**Рішення — використати збережений текст блоку** (`lastWrittenBlock`, METAFILE §6.3):

```
BEGIN є, END нема:
   якщо у стані є lastWrittenBlock для цього файлу
       і він буквально присутній у файлі  → вирізати САМЕ його, тоді splice як звичайно
   інакше                                 → дописати блок зверху + ГУЧНО залогувати
                                            + показати юзеру нотицю «у вашому .gitignore
                                            лишився пошкоджений блок, приберіть його
                                            вручну» (ми не знаємо, де він закінчувався)
```

Це і є друга причина зберігати в стані **текст**, а не хеш (METAFILE §6.3): хеш тут
безсилий, бо ним не можна нічого знайти у файлі.
...
| `END` є, `BEGIN` нема | те саме, що «BEGIN є, END нема», дзеркально: осиротілий `END` вирізати за `lastWrittenBlock`, інакше — нотиця |
...
- **Крок A — permission.** dot-hide як **фізичний блок** у `ROOT_INVARIANT_BLOCK`
  ...
  чужі `plugins/*/.gitignore`), запис стану `{mtime,size,lastWrittenBlock}` в окремому
  ...
  перевірки інваріантів. Тести: `lastWrittenBlock` розійшовся при незмінному файлі →
  ...
  **Плюс полагодити `spliceInvariantBlock` при осиротілому маркері (§3.1.3):** сьогодні
  ...
  за last-match. Лікування — вирізати за збереженим `lastWrittenBlock`, інакше нотиця.
