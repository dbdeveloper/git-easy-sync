...
> Сховище freshness-міток — [`SYNC2-METAFILE-REFACTOR.md`](./SYNC2-METAFILE-REFACTOR.md)
> §6 (окремий файл у `.runtime/`, запис = `{mtime, size, blockSha, blockLen}`). Тут — сам
> прохід.

**Множина файлів** (та сама, що в METAFILE §6.2): root `.gitignore`,
...
змінює саму константу блоку, не доїде до диска (METAFILE §6.3; це рівно той баг, через
який short-circuit колись і зняли).
...
відокремлена від змінної, і (б) `blockSha`/`blockLen` мають що міряти (METAFILE §6.3) —
...
**Рішення — знайти старий блок за парою «довжина + SHA»** (`blockLen`/`blockSha`,
METAFILE §6.3). Повний потік при апгрейді, для КОЖНОГО керованого `.gitignore`:
...
Крок 3 працює саме тому, що `blockLen`/`blockSha` міряні від **тіла БЕЗ маркерів**
(METAFILE §6.3): відсутній `END` просто не входить у вимірюваний відрізок. Передумова —
...
- Файл стану в `.runtime/` пишеться так само атомарно (METAFILE §6.4), але з іншої
  причини: там ідеться лише про те, щоб не ловити parse-помилку на торн-JSON.
...
- **Крок A — permission.** dot-hide як **фізичний блок** у `ROOT_INVARIANT_BLOCK`
  ...
  файлі `.runtime/` (METAFILE §6), і **виклик з ДВОХ точок — commit І drain**: сьогодні
