...
> §6 (окремий файл у `.runtime/`, запис = `{mtime, size, blockFingerprint}`). Тут — сам
> прохід.
...
для кожного файлу з множини:
    ...
    збіглися mtime + size + blockFingerprint?  → нічого не робимо
...
⚠️ `blockFingerprint` у порівнянні **обов'язковий** — без нього апгрейд плагіна, що
...
- **Крок A — permission.** dot-hide як **фізичний блок** у `ROOT_INVARIANT_BLOCK`
  ...
  чужі `plugins/*/.gitignore`), запис стану `{mtime,size,blockFingerprint}` в окремому
  ...
  перевірки інваріантів. Тести: `blockFingerprint` розійшовся при незмінному файлі →
