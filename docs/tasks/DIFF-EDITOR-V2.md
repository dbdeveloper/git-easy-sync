...
7. UI/UX виділення diff-groups:
   ...
   c. Зони diff-group ("<<<<<", "=====", ">>>>>") мають особливе значення для виділення. Selection, які починаються на
      цих зонах (тобто, клац мишки на цих зонах з затиснутою клавішою `[Shift]`, під "затиснутою клавішою `[Shift]` при
      виділенні тексту мишкою я маю на увазі клац і утримання лівої клавіші миші), і selection, які закінчуються на цих
      зонах (тобто, клавіша `[Shift]` відпускається, коли вказівник мишки знаходиться на цих зонах) мають дуже чітке
      значення, яке допомагає чітко і точно виділяти потрібні області тексту навіть коли саме позиціювання мишкою
      не дуже точне. Ось таблиця реакцій на різні selection, які починаються чи закінчуються в зонах diff-group
      ("<<<<<", "=====", ">>>>>"):

      i. selection початий десь НАД diff-group і який закінчився на зоні "<<<<<" даної групи означає: "виділити
         і додати до selection фінальний `\n`, останньої normal line, тієї що знаходиться безпосередньо 
         ПЕРЕД diff-group (тобто фактично ми на останній normal line робимо еквівалент команди `[Shift]+[End]`). 
         Те, що selection закінчився в зоні "<<<<<" (тобто в цій зоні користувач відпустив клавішу `[Shift]`) вказує на 
         те, що НЕ ПОТРІБНО ніяк виділяти саму diff-group (говоримо про саму diff-group, а не ver1-block, тому, що до 
         normal lines ми не маємо додати diff-group або в повному складі, або взагалі не додавати (правило 2.2.6 п.2); 
         Якщо ж при цьому виділення продовжиться нижче, тобто мишка переміститься НИЖЧЕ зони "<<<<<" (мишка курсора при 
         натисненій клавіші `[Shift]` опиниться НИЖЧЕ зони "<<<<<"), тоді вже УСЯ DIFF-GROUP ВИДІЛЯЄТЬСЯ І ДОДАЄТЬСЯ ДО 
         selection;

      ii. selection початий десь ПІД diff-group і який закінчився на зоні ">>>>>" даної групи означає: "виділити і додати
         до selection початок першої normal line, той що знаходиться безпосередньо ПІД diff-group, в зоні ">>>>>" 
         якого selection закінчився, не виділяючи ніяк саму diff-group; Фактично на цій першій після diff-group normal 
         line робимо еквівалент копманди `[Shift]+[Home]`. Якщо при цьому виділення продовчиться вижче, тобто
         мишка переміститься ВИЩЕ зони ">>>>>" (мишка курсора при натисненій клавіші `[Shift]` опиниться ВИЩЕ зони
         ">>>>>", тоді УСЯ DIFF-GROUP ВИДІЛЯЄТЬСЯ І ДОДАЄТЬСЯ ДО selection;

      iii. selection початий на зоні "<<<<<" і закінчений будь-де в межах ver1-block (тобто від "<<<<<" до "=====" 
          включно) чітко сигналізує що виділено повністю ver1-block. Отже, якщо користувач почав виділення 
          (клікнув мишкою і потягнув її вниз з затисненою клавішою `[Shift]`) на рядку "<<<<<", тоді вже через пробіг 
          мишки десь в висоту одного рядка в пікселях, УВЕСЬ ver1-block підсвічується як виділений, ЗА УМОВИ, ЩО
          користувач закінчив selection в межах ver1-block (між зонами "<<<<<" та "=====" включно). 
          Якщо ж користувач продовжить виділення вниз і ПЕРЕТНЕ зону "=====" в центрі diff-group, тоді вже УСЯ 
          DIFF-GROUP вважається повністю виділеною, за умови, що selection буде закінчене будь-де в межах ver2-block 
          (між зонами "=====" і ">>>>>" включно). Якщо selection продовжиться вниз далі, тоді selection включить в себе 
          УСЮ DIFF-GROUP і всі наступні за нею блоки тексту, поки selection не буде зупинено.

      iv. selection початий на зоні ">>>>>" і закінчений будь-де в межах ver2-block (тобто від "=====" до ">>>>>"
          включно) чітко сигналізує що виділено повністю ver2-block. Отже, якщо користувач почав виділення
          (клікнув мишкою і потягнув її вгору з затисненою клавішою `[Shift]`) на рядку ">>>>>", тоді вже через пробіг
          мишки десь в висоту одного рядка в пікселях, УВЕСЬ ver2-block підсвічується як виділений, ЗА УМОВИ, ЩО
          користувач закінчив selection в межах ver2-block (між зонами "=====" та ">>>>>" включно).
          Якщо ж користувач продовжить виділення вгору і ПЕРЕТНЕ зону "=====" в центрі diff-group, тоді вже УСЯ
          DIFF-GROUP вважається повністю виділеною, за умови, що selection буде закінчене будь-де в межах ver1-block
          (між зонами "<<<<<" та "=====" включно). Якщо selection продовжиться вгору далі, тоді selection включить в себе
          УСЮ DIFF-GROUP і всі блоки тексту НАД нею, поки selection не буде зупинено.
      ...
      v. selection початий в межах текстових рядків ver1-block (тобто між "<<<<<" і "=====" НЕ включно), виділяють 
         ...
         чи "=====", одразуж УВЕСЬ ver1-block вважається виділений! Подальший рух виділення вгору/вниз відбувається
         так само як в п7.iii)

      vi. selection початий в межах текстових рядків ver2-block (тобто між "=====" і ">>>>>" НЕ включно), виділяють
          безпосередньо частину тексту ver2-block. ОДНАК, ЯК ТІЛЬКИ мишка при цьому selection потрапляє в зони "====="
          чи ">>>>>", одразуж УВЕСЬ ver2-block вважається виділений! Подальший рух виділення вгору/вниз відбувається
          так само як в п7.iv)

   e. при виділенні не мишкою, а з допомогою клавіатури, правила можуть дещо відрізнятись від тих, які ми бачимо при
      виділенні мишкою.

      і. selection розпочатий в normal-lines НАД diff-group при переході кордону diff-group
         ...
         diff-group (2.2.6 п.7g). І тільки, якщо буде виділено ХОЧА Б ОДИН реальний СИМВОЛ diff-group — ВСЯ diff-group 
         вважається виділеною (2.2.6 п.2). Для empty ver1-block це означає, що якщо selection почався вище diff-group
         і закінчився на позиції 0 empty ver1-block, то виділення стосується тільки normal lines ВИЩЕ diff-group, і 
         тільки розширення selectioin далі ([Shift]+[Down]/[Rigt]/[PgDn]) при якому курсом вже переміститься на 
         vер2-блок (бо ver1-block порожній), тільки за такої дії УСЯ diff-group об'являється виділеною:
   ...
   f. click мишкою по рядку "=====" з затиснутою кнопкою `[Shift]` (коли ще не ясно що буде виділено) очікує рух мишки
      вниз чи вгору по тексту. Якщо мишка рухнулась вверх (хоча б на відстань висоти одного рядка), тоді УВЕСЬ ver1-block 
      помічається (як вказано в п.7c цього розділу) як виділений. Якщо ж мишка рухнулась вниз (хоча б на відстань висоти 
      одного рядка), тоді УВЕСЬ (як вказано в п7c цього розділу) ver2-block помічається як виділений.
...
Після порівняння двох файлів з допомогою diff2() ми отримує файл, в якому diff-groups обов'язково розділені normal lines.
Однак, так як ми дозволяємо редагувати цей diff-file в нашому diff-editor v2, можливі випадки, коли два чи більше
diff-groups розташовуються разом, а отже, щоб не порушувати правило: "дві diff-groups ЗАВЖДИ розділені одинм або кількома
...
Користувач може вільно редагувати будь-який текст в diff-document, представлений в diff-editor-v2, а отже можливі випадки,
...
Однак це тільки один із можливих розв'язків. При редагування ver-blocks може виникати безліч ситуацій проміжних розв'язків,
наприклад, за якихось обставин велика diff-group може розбитись на дві, коли в результаті редагування всередині її з'являться
...
3. Відключається чутливість зони "<<<<<", ">>>>>" на клік мишкою, tap по touchscreen, після якої відкривається для редагування 
...
Conflicts: NNN [  ↑ ] [ ↓  ]  Auto-focus [x]
Undo:      MMM [Undo] [Redo]  Touch-mode [x]
```
NOTE:  `[<-]`, `[ ↑ ]`, `[ ↓ ]`, `[Undo]`, `[Redo]` - це графічні зображення цих дій (іконки), а не символи

перенос кнопок при зменшенні ширини вікна відбувається ПО РЯДКАМ з вирівнюванням вправо. Тобто кожний рядок
...
Auto-focus стосується conflicts і ЗАВЖДИ фокусується тільки НА ПЕРШОМУ конфлікті в документі. Якщо тут увімкнено,
тоді коли вирішується якийсь конфлікт, то на екрані показується знову перший конфлікт. Якщо закрити перший конфлікт,
то другий стає першим і відповідно автоматично скролінг документа висвітлює другий конфлікт на екрані, при його
вирішенні третій конфліт стає "першим" і так далі... Якщо пропустити перші 10 конфліктів і вирішити 11-й, тоді
при увімкненому "Auto-focus: `[x]`", документ знову покаже перший конфлікт. Якщо це небажана поведінка, користувачу
потрібно виключити цей checkbox. Перед редагуванням. Він потрібний для автоматичного швидкого вирішення конфліктів, 
швидше за все, на мобільних приладах.

Touch-mode дивись п.2.2.14 

І "Touch Mode", і Auto-focus мають свою (default) значення, які користувач проставляє в settings (які встановлюються у 
false при першій ініціалізації data.json цього plugin), але для відкритого документу їх можна змінювати
саме для цієї сесії редагування цього документу. Якщо документ закрити і знову відкрити в diff-editor ці checkbox
