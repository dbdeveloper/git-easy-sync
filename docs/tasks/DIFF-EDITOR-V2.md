...
> **✅ СТАТУС (2026-06-27): ця специфікація ПОВНІСТЮ реалізована** (device-verified, гілка
> `fix-diff-editor`, pushed @ `1a928e3`). Уся interaction-модель (§2.2.1–§2.2.16) + touch-only
> (§2.2.14) + toolbar (§2.2.15) живі. **Нові фічі заморожено** (баги ще шукаємо й правимо).
...
7. UI/UX виділення diff-groups:
   ...
   c. Маркери diff-group ("<<<<<", "=====", ">>>>>") мають особливе значення для виділення. 
      ...
      v. selection початий в межах текстових рядків ver1-block (тобто між "<<<<<" і "=====" НЕ включно), виділяють
         безпосередньо частину тексту ver1-block. ОДНАК,

         (a) ЯК ТІЛЬКИ мишка при цьому selection потрапляє на маркер "<<<<<", тоді selection_end одразу ж встановлюється
             в позицію 0 першого рядка ver1-block - виділяючи весь початок plain тексту ver1-block до місця початку 
             selection;

         (b) ЯК ТІЛЬКИ мишка при цьому selection потрапляє на маркер "=====", одразу ж кінець 
             selection встановлюється в позицію 0 першого рядка ver2-block - виділяючи весь кінець plain тексту 
             ver1-block (крім випадку, коли початок selection почався з позиції 0 першого рядка ver1-block, чи на
             маркері "<<<<<" - тоді вибраним вважається УВЕСЬ ver1-block! Подальший рух виділення вгору/вниз 
             відбувається так само як в п7.iii)

      vi. selection початий в межах текстових рядків ver2-block (тобто між "=====" і ">>>>>" НЕ включно), виділяють
         безпосередньо частину тексту ver2-block. ОДНАК,

         (a) ЯК ТІЛЬКИ мишка при цьому selection потрапляє на маркер "=====", тоді selection_end одразу ж встановлюється
             в позицію 0 першого рядка ver2-block - виділяючи весь початок plain тексту ver2-block до місця початку 
             selection;
         (b) ЯК ТІЛЬКИ мишка при цьому selection потрапляє на маркер ">>>>>", одразу ж кінець 
             selection встановлюється в позицію 0 першого normal line ПІСЛЯ цієї diff-group- виділяючи весь кінець 
             plain тексту ver2-block (крім випадку, коли початок selection почався з позиції 0 першого рядка ver2-block,
             чи на маркері "=====" - тоді вибраним вважається УВЕСЬ ver2-block! Подальший рух виділення вгору/вниз 
             відбувається так само як в п7.iv)
...
#### 2.2.14 Режим Edit-Mode vs Touch-Mode (“Read-only” mode with mouse clicks/taps on the touchscreen enabled)
...
4. Selection з допомогою мишки залишається. працює CTRL+C - копіювання selection в clipboard. Вставка/видалення 
   selection НЕ ПРАЦЮЄ
...
Кнопки toolbar  `[ ↑ ]` і  `[ ↓ ]` в diff-editor-v2 мають мати свої hotkeys: `CTRL+[` - для `[ ↑ ]` (попередній конфлікт)
і `CTRL+]` - для `[ ↓ ]` (наступний (відносно позиції курсору) конфлікт).
...
- **Character** (default) — підсвічуються ТІЛЬКИ змінені символи. Напр. `True` vs `true` → лише `T`/`t`;
  `false` vs `true` → `fals`/`tru` (спільне фінальне `e` не чіпається). Точно.
- **Word** — підсвічуються ЦІЛІ змінені слова (`True`/`true` → ціле слово на кожній стороні).

ВИНЯТОК (завжди, НЕЗАЛЕЖНО від обраного режиму): на ВЕЛИКИХ diff-groups (добуток довжин сторін
перевищує поріг ~200k символів) character-level diff (складність O(n·m)) задорогий для перерахунку
на кожне натискання → автоматично вмикається Word-режим.

Default — Character (див. також п.2.2.15: defaults у settings + per-session перемикач у toolbar).
Зараз вмикається в Settings → "Diff Editor" → dropdown "Diff highlight mode"; toolbar-перемикач —
разом із редизайном toolbar (п.2.2.15).

#### 2.2.18 Адаптивний layout маркер-панелей (`<<<<<` / `=====` / `>>>>>`)

Рядок маркера — це `[гліф] [кнопки резолву] [(мітка-пристрою)]`. Задача: щоб на будь-якій ширині
редактора (desktop split, mobile portrait/landscape) панель поводилась ПРОГНОЗОВАНО й компактно, а не
розсипалась стовпчиками. Реалізація: `MarkerWidget` / `appendMarkerParts` (`diff-pane-v2.ts`),
`marker-layout.ts` (чиста `chooseMarkerMode`), `markerLayoutController`, `.diff2-marker*` (`styles.css`).

**Монолітні атоми (flow-wrap, принцип toolbar §2.2.15).** Гліф, КОЖНА кнопка й мітка — атоми в ОДНОМУ
`flex-wrap`-потоці, кожен `flex:0 0 auto` + `white-space:nowrap`. Тому гліф `<<<<<` ніколи не ділиться на
одиночні `<`, кнопки не стискаються, а коли не влазить — останній атом (спершу мітка, потім задня кнопка)
переноситься на наступний рядок ЦІЛИМ. Гліфи — 5-символьні. Мітка — жирна (tag-like), просто
переноситься на власний рядок (без ellipsis).

**Два важелі компактності** (тільки вони змінюють потрібну ширину):
- **іконки** — кожна кнопка несе ДВА варіанти: `full` (слово+стрілки, напр. `Keep ↓` / `> Join ↓`) і
  `compact` (lucide-іконка дієслова + стрілки: `check`=keep/apply/restore, `trash-2`=remove; Join = `> ↓`
  без іконки). Стрілки ↓/↑/↓↑ **завжди** видимі (несуть напрям). Клас `.diff2-btns-icon` на корені редактора
  перемикає, який варіант показано. Повне слово лишається в `aria-label` + tooltip.
- **зсув у gutter** — навпроти рядка маркера НЕМА номера рядка, тож на вузькому всю панель зсуваємо ВЛІВО
  в зону gutter (`.diff2-marker-slid` → `margin-inline-start: -var(--diff2-gutter-w)` + `z-index` понад
  sticky-gutter CM6), відвойовуючи ширину gutter. `--diff2-gutter-w` публікує контролер.

**Пріоритети (від вищого):** 1) усе в ОДИН рядок (мітка inline, не перенесена); 2) ТЕКСТ на кнопках
(зрозуміліше за іконки); 3) звичайний відступ від gutter. Жертвуємо найнижчим першим: спершу зсув, потім
іконки, і аж останнім — один рядок (перенос мітки). При зміні важеля — ПЕРЕРАХУНОК з початку (на іконках
відступ повертається, бо іконки вужчі).

**Драбина `chooseMarkerMode(avail, widths)`** — перший рядок, що влазить (`slidAvail = avail + gutter`):

| # | один рядок | кнопки | відступ | влазить якщо |
|---|---|---|---|---|
| 1 | inline | текст | звичайний | `labelRowText ≤ avail` |
| 2 | inline | текст | зсув | `labelRowText ≤ slidAvail` |
| 3 | inline | іконки | звичайний | `labelRowIcon ≤ avail` |
| 4 | inline | іконки | зсув | `labelRowIcon ≤ slidAvail` |
| 5 | wrap | текст | звичайний | `buttonsText ≤ avail` |
| 6 | wrap | текст | зсув | `buttonsText ≤ slidAvail` |
| 7 | wrap | іконки | звичайний | `buttonsIcon ≤ avail` |
| 8 | wrap | іконки | зсув | (крайній fallback) |

Рядки 5–8 = 1–4 із прибраною міткою (мітка вже на власному рядку → line-1 = гліф+кнопки). Перенос мітки —
автоматичний per-row flow-wrap: у станах 1–4 мітка гарантовано влазить (умова її й перевіряє), у 5–8 — ні,
тож падає на 2-й рядок сама. **Наслідок пріоритетів:** можливий «фліп» текст→іконки→текст у вузькій смузі
(де іконки тримають один рядок, а текст уже ні) — ПРИЙНЯТО (строго за пріоритетами; на пристрої з фіксованою
шириною не видно, лише при безперервному drag-resize вузького desktop-split).

**Вимір — КОНСТАНТИ.** Усі шматки константної ширини (monospace-шрифт, фіксовані рядки кнопок, мітки-пристроїв
однакові для всіх diff-group у редакторі, gutter росте лише з кількістю цифр номера). `measureMarkerWidths`
раз міряє offscreen-клони 3 видів маркера у 4 конфігураціях (текст/іконки × з міткою/без) → глобальні
`labelRowText/labelRowIcon/buttonsText/buttonsIcon` = максимум по видах. Рішення ГЛОБАЛЬНЕ: з ширини одразу
відомо, як виглядатимуть УСІ diff-group.

**Контролер.** `markerLayoutController` (ViewPlugin) на кожну реальну зміну ширини (`ResizeObserver` на
`.cm-content` — не per-frame) міряє свіжо, читає ЖИВУ `avail = contentDOM.clientWidth` + gutter, рахує
`chooseMarkerMode`, вішає `.diff2-btns-icon` / `.diff2-marker-slid` на `view.dom`. Жива ширина (а не запечені
CSS-пороги) лишається коректною коли з'являється вертикальний scrollbar або gutter росте на цифру під час
розв'язання. Re-apply також на rAF / `document.fonts.ready` (CSS/шрифти можуть застосуватись після першого
layout). Ключ `avail:gutter:widths` відсіює зайві прогони.
