...
4. ЗАСТАРІЛО! Додати в контекстне меню каталогів Obsidian пунк меню: ".gitignore" — відкривати й редагувати `.gitignore`
   ...
   НОВА УМОВА: Так, як в нас насправді є тільки root `.gitignore` (.gitignore в `.obsidian/**` до уваги не беремо, як і в
               ...
               ДОПОВНЕННЯ (2026-08-28, знайдено при аудиті "тихих" проблем, D6a SYNC2-DOT-FILES-REFACTOR.md): цей
               редактор — єдиний майбутній UI-шлях до root `.gitignore` для пересічного користувача (сьогодні файл
               фізично недосяжний через Obsidian, дістатись можна лише зовнішнім інструментом — це вже саме по собі
               відсікає ВИПАДКОВУ дію). Тому редактор має нести два додаткові запобіжники проти "тихого"
               саморозшарення (D6a: рядок `/.gitignore` нижче керованого блоку робить сам файл per-device — усі
               `!`-правила користувача при цьому перестають синкатись, і якщо це станеться на ВСІХ пристроях, бекапу
               опцій ніде нема):
               1. Обов'язкове модальне попередження ПЕРЕД відкриттям редактора root `.gitignore` із Settings —
                  блокує показ, доки юзер не натисне явне підтвердження ризику (напр. "Зрозумів. Ризики беру на
                  себе") — той самий контракт двокрокового підтвердження, що вже прийнятий для Reset
                  (SYNC2-RESET-PLUGIN.md).
               2. Контекстна підказка ВСЕРЕДИНІ самого редактора (не лише одноразовий вхідний гейт) — підсвічує й
                  пояснює рядок `/.gitignore` конкретно в момент, коли юзер його бачить чи вписує, а не покладається
...
49. Дослідити: `evaluateConflictState` ставить `ConflictWatcher` на паузу на час скану (SYNC2-FIX.md §8.7, п.6,
    рядки ~1009-1013). Паралельний commit-coalesce цикл (дозволений під час drain, R3a/R3b) міг би переплестись із
    цим вікном паузи/відновлення — сам документ каже "Відкрите; поки без окремого тесту". Якщо стан
    паузи/відновлення лишиться неузгодженим через таке переплетення, vault-зміни в цьому вікні можуть тихо
    загубитись замість помилки. Знайдено під час аудиту "тихих" проблем 2026-08-28 (SYNC2-* документи).

50. ✅ DONE (2026-08-28, `49f2d95`). Знайдено під час рецензії готовності SYNC2-NEW-DRAIN.md до імплементації:
    3-way merge (`three-way-merge.ts` `pickSeparator`, використовується сьогоднішнім двигуном через
    `conflict-detection.ts`, і дзеркально — `cpu-worker.ts` для worker-потоку) обирав CRLF, якщо CRLF був у
    БУДЬ-ЯКОМУ з трьох входів (`ours`/`base`/`theirs`) — тобто чужий `base`/`theirs` міг тихо перевернути
    LF-файл користувача на CRLF після ЧИСТОГО merge (без жодного конфлікту, без сигналу). Виправлено: результат
    тепер зберігає ВЛАСНИЙ домінантний EOL-стиль `local` (`ours`), не "будь-який вхід із CRLF" — той самий,
    вже готовий і протестований алгоритм (`detectEol`/`restoreEol`, bug-59), перенесений із `src/diff2/eol.ts`
    у новий канонічний `src/sync2/eol.ts` (`diff2/eol.ts` тепер лише ре-експортує). Обидві копії
    (`three-way-merge.ts` + `cpu-worker.ts`) виправлено синхронно. RED-GREEN тести:
    `three-way-merge.test.ts` A.27-29. Рішення й обґрунтування — `docs/tasks/SYNC2-NEW-DRAIN.md` §III
    (`_diff3` CRLF residual case).

51. ✅ DONE (2026-08-29, `8a082bb`). Знайдено під час того самого аудиту готовності: повний прогін
    `pnpm test:integration` дав 9 провалів (2 родини: "не та сторона перемагає" при резолюції конфлікту —
    `branch-lifecycle`, `defer-then-resolve-via-sibling-delete`, `edit-while-in-conflict`,
    `multi-copy-pair-resolution`, `G3-same-line-conflict`, `G4-binary-atomic-across-devices`; і "осиротілий
    запис у snapshot" — `D6-same-file-local-delete-remote-modify`, `D8-same-file-deleted-both`). Через
    `git bisect` (двічі, незалежно, на двох різних тестах) локалізовано до одного коміту — `3a98c96` (SYNC2
    §7.10, "chaining", 4 липня). Корінь: `chainedHead ?? pullIfNeeded()` пропускає `pullIfNeeded()`, коли
    `chainedHead` непорожній — а він ЗАВЖДИ непорожній після будь-якого `processBatch`, включно з випадком
    "batch спорожнів після reconcile" (усі шляхи пішли в конфлікт або дублікат-видалення). Пропущений виклик
    — не формальність: САМЕ він, у своїй пост-циклевій логіці, безумовно просуває `lastSyncCommitSha` до
    живої голови, коли жоден шлях не був "відкладений" — незалежно від того, чи результат по файлу був
    "чистий apply" чи "register-conflict". Без нього `lastSyncCommitSha` застряє на до-конфліктному коміті
    НАЗАВЖДИ — кожна наступна правка того самого шляху заново дає ТОЙ САМИЙ конфлікт (підтверджено живцем: 6
    поспіль `register-conflict` там, де мав бути 1), а осиротілий запис snapshot ніколи не чиститься.
    Перша гіпотеза (деградація `getGuardedHead()`) виявилась хибною — реалізована, unit-протестована,
    відкинута після живої перевірки (жодного впливу на 9 провалів); задокументована в `SYNC2.md` §7.14 як
    застереження. Фікс: `processBatch` тепер повертає, чи справді відбувся push; `chainedHead` довіряється
    лише тоді. RED-GREEN unit-тест (детермінований, без реальної мережевої гонки) + живий повторний прогін:
    усі 9 тестів окремо, і повний `pnpm test:integration` (82 файли/137 тестів) — зелені.

52. якщо batch повністю обробився і запушився на сервер, а потім був збій, тоді в metadata.files не змінились base, а 
...
## Layer 2 після кочення голови — перечитувати дерево замість per-path HEAD (sync2, perf) — ВІДКЛАДЕНО 2026-09-01

**Контекст.** Основну частину вже зроблено (`476262f`, спека `SYNC2-NEW-DRAIN §II.13`): коли
discovery читав повне дерево, Шар 2 бере `{sha,size}` зі знімка `{atCommit, paths}` замість
одного HEAD на файл. Умова — `atCommit === head_hash`. На адопції це дало **255 → 0** запитів,
90.1 → 5.3 с у мережі, ~87 → 7.1 с wall.

**Що лишилось незакритим.** `head_hash` **котиться після кожного пушу батчу**, а батчі нарізані
по 100 (`MAX_BATCH_ENTRIES`). Тож знімок покриває лише батчі ДО першого пушу; далі — знову HEAD
на кожен файл.

**Замір найгіршої форми (2026-09-01, копія `~/Obsidian-test` + 500 нових файлів, реальний GitHub):**

```
759 локальних файлів → 8 батчів
прохід 1:  699 запитів,  206 с у мережі,  209 с wall
           655 × contents-head   184 с    ← 89% часу
GET tree/<sha>?recursive=1        443 ms   80.1 KB   (259 блобів ≈ 309 Б/блоб)
HEAD contents-head                281 ms   × 655
```

**Рішення (узгоджено, НЕ реалізовано).** Коли знімок застарів і в батчі **≥ 20** записів —
один `getRepoTree(head_hash)`, перебудувати знімок, далі весь батч безкоштовно. Менше 20 —
лишаємось на HEAD (при 20 записах це ~5.6 с проти одного читання; нижче різниця не варта
завантаження дерева). Очікуваний ефект на заміряній формі: 8 читань замість 655 HEAD,
≈206 с → ≈25 с.

**Два запобіжники, без яких зміна шкодить:**
1. **`truncated` тут НЕ помилка.** У discovery обрізане дерево кидає `TreeTruncatedError` — і це
   правильно. Тут воно має означати «знімка немає, йдемо по HEAD», інакше велике сховище ламатиме
   пуш замість того, щоб просто працювати повільніше.
2. **Дуже великі дерева не перечитуємо.** Знімок знає свій розмір; при >10 тис. блобів (≈3 МБ на
   читання) вигідніше лишитись на HEAD. ⚠️ Ця межа **не міряна** — жодного свідчення, що хтось у
   неї впирається, тож проєктувати під неї не треба, але й погіршувати не можна.

**Технічно.** `getRepoTree` є в `GithubClient` і в `drain-deps`, але **не** в інтерфейсі
`DrainClient` (`drain.ts`) — його треба туди додати, і у фейк `drain-harness.makeClient()`.

**Свідомо ВІДХИЛЕНА альтернатива — прокотити знімок уперед власним пушем** (нуль запитів).
Аргумент на її користь сильний: PATCH рефа non-force, тож успіх означає, що між нашим читанням і
пушем ніхто не втрутився (інакше 422). Але тоді Шар 2 звіряється з тим, що ми **самі** порахували,
і помилку в нашому ж обліку записаного вже не спіймає — а це рівно те, заради чого він існує.
Перечитування дерева цю незалежність зберігає й коштує копійки.

---

## Прогрес-повідомлення в новому drain — де їх тепер вставляти (UX) — ВІДКЛАДЕНО 2026-09-01

**Скарга з поля (власник, після живого bootstrap на `~/Obsidian-test`):** у старому плагіні були
«чудові прогрес-повідомлення», у новому drain їх немає.

**Чому це не механічний перенос.** Алгоритм інший: стадії старого двигуна (pull → merge → push)
більше не існують. У `drainOnce` природні точки — інші: клейм батчу, per-entry цикл (де вже є
file-count progress), пуш батчу (їх N, а не один), Vault-step, епілог (baseline transfer, sweep).
Ще й кількість батчів наперед відома лише приблизно.

Тобто спершу треба **вирішити модель прогресу** (що саме показуємо: файли? батчі? стадії?), а вже
потім вибирати місця в коді. Окрема розмова, окремий крок.

---

## Два відкриті рішення, що блокують/стосуються деплою (вказівники, не дублікати)

- **🔴 `SYNC2-DOT-FILES-REFACTOR.md §8.0`** — `enforce()` пише керовані `.gitignore` ДО першого
  скану, тож на холодному старті його власний запис виглядає як правка юзера без бази → ручний
  конфлікт на root `.gitignore`. Відтворено і на новому порожньому vault, і на живому пристрої
  власника. **Блокує живий cold-start на `main`.**
- **🟡 `SYNC2-DOT-FILES-REFACTOR.md §3.4.1 дефект 1`** — тумблер «Push plugins data.json» —
  no-op на кожному vault, де файл створював наш плагін (наш блок стоїть ВИЩЕ нашого ж
  `plugins/*/*`, а gitignore — last-match-wins). Fail-safe, деплой не блокує.
