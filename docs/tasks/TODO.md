...
47. Дивний код у детекторі змін: watermark-гейт `change-detector.ts:211-233` схоже що не робить нічого. Знайдено
    ...
    ЗВІДКИ ВЗЯЛАСЯ НАДЛИШКОВІСТЬ (це не помилка початкового дизайну, а слід від пізнішого фіксу). Гейт
    замислювався ЖОРСТКИМ фільтром — коментар `:207-209` досі каже «Pass 1: candidates whose stat.mtime
    exceeds the watermark», тобто «не перевищив → не дивимось узагалі». Але його довелося послабити:
    за коментарем `:213-217`, пропустити файл можна лише маючи ДОКАЗ, що він справді збігається з останнім
    синком, — бо `gitignore`-правило могло відтоді змінити статус шляху, і `isSyncable` треба питати заново.
    Так замість `continue` з'явився `// Fall through`. А шлях, у який тепер провалюємось, уже містить ту саму
    stat-перевірку (`:276-284`) — і саме це зробило гейт надлишковим. Колись він працював; після послаблення
    перестав, але лишився на місці.
