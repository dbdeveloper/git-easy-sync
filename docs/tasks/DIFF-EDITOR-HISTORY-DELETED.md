...
- ⚠️ **`liftForCompare` / `returnFromCompare` (`trash-store.ts`) — у цій секції були
  ПРОПУЩЕНІ, хоч це не дрібниця.** Там сесійна модель: `liftedAsSessionId` і охорона
  «вже піднято іншою сесією», тобто операція «підняти байти з кошика у робочу сесію
  порівняння». У контент-адресованому сховищі **підіймати нема чого** — байти
  незмінні й спільні, а «зайнятість однією сесією» втрачає сенс. Re-platform мусить
  явно вирішити: операція стає no-op, чи міняє значення (наприклад, лишається лише
  облік «який сеанс дивиться цей sha»).
