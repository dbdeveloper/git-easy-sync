...
- ⚠️ **`trash-recovery.ts` — теж поза первісним описом.** Він існує рівно щоб
  полагодити недописану копію в `.trash/<id>/vault/`. Під `sync_store` запис
  контент-адресований і перевіряється хешем при читанні, тож модуль найімовірніше
  помирає цілком — але це має бути записане рішення, а не побічний ефект.
