...
>
> **Лишилось:**
> - ⛔ **прибирання старого кошика ЗАБЛОКОВАНО відкритим питанням retention** — див.
>   «Ревізія R3.5-TTL» нижче. `TrashStore`/`trash-recovery`/`.trash/` більше ніхто не
>   ПИШЕ (обидва записувачі переведені), але їхні шари 1b і 2 досі єдині, хто прибирає
>   записи, які НІКОЛИ не потраплять у батч.
> - ⛔ **щит сесії перегляду** (носій + чи лишати ексклюзивність) — рішення власника.
> - `restore(id)` + UI — Фаза 9b, поза цим воркстрімом.
>
> ### ⚠️ Ревізія R3.5-TTL — питання, яке відкрив крок 3 (НЕ вирішено)
>
> Передача в батч прибирає запис лише для тих видалень, які **доходять до коміту**. А
> три шари старого кошика існували рівно тому, що частина видалень туди не доходить
> ніколи — це дослівно з коментаря шару 2: «orphan/synthetic siblings, gitignored
> deletes (.log etc), pull-delete entries from earlier drains». Такі шляхи
> **не синкаються за визначенням** (`isSyncable` забороняє sibling-и за патерном,
> `.gitignore` — решту), тож їхній запис у `deleted.json` не звільнить ніхто, і його
> блоб лишиться закріпленим назавжди.
>
> Отже просто видалити `TrashStore` = тихо прибрати retention. Потрібне рішення
> власника: **(а)** TTL за `deletedAt` у новому кошику (аналог шару 2, але за часом, а
> не за id-порогом); **(б)** не тримати взагалі записи для шляхів, які `isSyncable`
> відкидає (тоді sibling-и й `*.log` у кошик не потрапляють — дешево, але втрачається
> «випадково видалив лог, хочу назад»); **(в)** обмеження за кількістю/розміром.
> До рішення старий кошик лишається на місці — він нічого не псує (ніхто в нього не
> пише), але й не дає новому рости безмежно.
