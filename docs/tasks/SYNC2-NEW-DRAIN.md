...
1. Маємо metadata.files[A]={baselineSha, size, mtime} — тут зберігається інформація про файл на момент, коли цей файл
   ...
   знаходяться в дозволених dot-локаціях.
   ⚠️ **ЗВІРКА 2026-09-23 (§IX.2 п.1): «дозволені» — це вже НЕ «ті, яких нема в
   `.gitignore`».** Після DOT-FILES Кроку B2 дозвіл дає **opt-in-набір**
   (`readRootGitignore`: названі dot-ФАЙЛИ + анкеровані walk-targets, плюс
   `<configDir>/` за per-device перемикачем). Членство в наборі — умова D7: шлях поза
   набором не синкається взагалі, навіть якщо `.gitignore` його не забороняє. Канон —
   [`DOT-FILES §4.2`](./SYNC2-DOT-FILES-REFACTOR.md) і [`SYNC2.md §13`](../SYNC2.md). Цей список (в основному, але є винятки)
...
1. **Ім'я гілки персистується ДО першого мережевого виклику, що її торкається — не після.** Ім'я
   ...
   ```
   ⚠️ **ДЕ саме виконується цей блок — див. §II.7.1 (ліниве мінтування, 2026-09-23).**
   Порядок «ім'я → диск → мережа» лишається дослівно цим, але сам блок переїхав із
   seed-кроку в `shouldPushToConflictBranch()`, бо безумовний мінт коштував три зайві
   мережеві запити на КОЖНОМУ порожньому sync'у. Seed зберігає лише ВІДНОВЛЕННЯ імені
   (журнал → hot-пара).
...
### II.7.1 Мінтування імені — ЛІНИВЕ (польовий вимір 2026-09-23)

Псевдокод п.1 вище ставить мінт у seed-крок, безумовно. Польовий лог показав, чого це
коштує на найчастішому сценарії — **порожній drain**: нічого локального, remote не
рухався, конфліктів нуль.

```
стара версія (github-easy-sync):  1 запит,  ~0.5 с
нова  версія (git-easy-sync):     5 запитів, ~2.0 с   (447+390+425+380+253 мс)
```

П'ять послідовних round-trip'ів по ~400 мс; CPU в цьому не бере участі взагалі
(поза-мережевий час усього ~144 мс з 2039 мс). Чотири з п'яти запитів — зайві, і три
ростуть з одного кореня.

**Корінь.** Ім'я гілки конфліктів мінтиться БЕЗУМОВНО. У логу видно мітку часу того ж
запуску (`…-20260923000937-550`) — тобто drain вигадує ім'я, персистить журнал на диск
і йде в мережу питати `getBranchHeadSha` про гілку, яка **за побудовою не може
існувати**. Далі в епілозі ворота FINALIZE (§II.14) читають умову «ім'я є і
нерозвʼязаних конфліктів нема» — ім'я є, бо ми його щойно вигадали — і платять ще два
запити: свіжу головну голову й ту саму фантомну гілку. 404 → ім'я обнуляється → **і так
на КОЖНОМУ sync'у, вічно.**

**Чому ліниве мінтування не порушує §II.7.** Вимога п.1 сформульована відносно події:
ім'я на диску ДО першого мережевого виклику, що ТОРКАЄТЬСЯ гілки. Її мета названа там
же — щоб крах між «push удався» і «журнал записано» не лишив гілку-сироту. Цей ризик
існує рівно тоді, коли push може відбутись. Drain, який жодного разу не дійшов до сайту
push'а в гілку конфліктів, **не має чого осиротити**, і ім'я, вигадане для нього,
купує гарантію проти краху, який неможливий.

Тому мінт переїжджає в `shouldPushToConflictBranch()` — єдину функцію, через яку
проходить КОЖЕН шлях до push'а в гілку (обидва сайти накопичення, STEP1 і STEP2,
викликають її першою). Вона ж і є тим «першим мережевим викликом, що торкається
гілки», тож порядок «ім'я → диск → мережа» лишається буквально тим самим:

```
def ensureConflictBranch():                 # раз на запуск (мемоізовано; скидається на 422-рестарті)
    if conflictBranchName is null:
        conflictBranchName = buildConflictBranchName(deviceLabel, now())
        persistDrainState()                 # §II.7 п.1 — ДО мережі, дослівно
    conflict_head_hash = getBranchHeadSha(conflictBranchName)

def shouldPushToConflictBranch(path, sha, …):
    if record confirms sha:  return false   # швидкий шлях — БЕЗ мережі, БЕЗ мінту
    ensureConflictBranch()                  # ось тут гілки вперше торкаються
    if conflict_head_hash is null: …        # далі без змін
```

**Чому персист усередині батчу не ламає W1/D.16.** Мінт тепер може записати журнал
у СЕРЕДИНІ обробки батчу, а не до неї — отже треба показати, що на диск не потрапить
нічого з «недороблених» станів. Потрапити може рівно одне: прапорці
`isManualConflict`, які вже стоять у `state` на цей момент. Усі вони — seeding-похідні
(§III крок 0), тобто для кожного durable-запис у `conflicts.json` ВЖЕ існує; це стан
«запис є, прапорець є», який таблиця чотирьох станів краху (`drain.ts`, W1) називає
коректним. Прапорець, народжений цією спробою (STEP1), піднімається ПІСЛЯ виклику
`ensureConflictBranch()` — мемоізація гарантує, що персист уже відбувся — і доходить до
диска лише парою «сховище→журнал» на МЕЖІ батчу, де W1 і впорядковує її store-first.
Стан «прапорець у журналі без запису у сховищі», єдиний руйнівний із чотирьох, цим
персистом недосяжний.

Seed-крок при цьому зберігає ВІДНОВЛЕННЯ імені (журнал → hot-пара, J.2) і втрачає лише
мінт та `getBranchHeadSha`. Наслідок для воріт FINALIZE: вони тепер спрацьовують лише
на імені, що пережило ПОПЕРЕДНІЙ запуск, — тобто на гілці, яка справді могла існувати.
Самолікувальний шлях 404 («гілку вже видалено, почисти поле») працює як працював, але
платиться один раз, а не щосинку.

**Другий корінь — епілог не бачить збереженого дерева.** Крок 3 епілогу вирівнює пару
(commit, tree) і, коли drain нічого не пушив, платить `getCommit` за tree sha. Логіка
правильна (перекіс заборонений METAFILE §2.1.2), але в найчастішому випадку голова не
рухалась: `headHash == baseHash`, і hot-пара вже тримає дерево саме для цього коміту.
`HotMetadata.getLastSyncTreeSha()` існує — його просто не пропущено через інтерфейс
`hot` у drain. Тому:

```
if headHash is not null and knownHeadTreeSha is null:
    if headHash == hot.getLastSyncCommitSha():      # голова не рухалась цього запуску
        knownHeadTreeSha = hot.getLastSyncTreeSha() # пара вже вирівняна — питати нема чого
    if knownHeadTreeSha is null:                    # (збережене дерево теж null → чесно платимо)
        knownHeadTreeSha = getCommit(headHash).tree.sha
```

**Разом:** порожній drain = 1 запит, як у старій версії. Жоден із чотирьох прибраних
запитів не ніс інформації, якої цей запуск не мав би без нього.
...
> ⚠️ **ЗВІРКА 2026-09-23 (§IX.2 пп. 2-3) — дві речі з'явились після написання секції.**
> **(а) Scope discovery має власний lifecycle:** `ChangeDetector.beginScan()` фіксує
> opt-in-набір на ВСЮ операцію, `endScan()` (у `finally`) його звільняє, а `isSyncable`
> **кидає**, якщо scope не встановлено. Drain кличе `beginScan()` до discovery —
> всередині `try` і після спалаху статусу. Обґрунтування — DOT-FILES §5.
> **(б) Порожнє дерево — не помилка:** `getRepoTree` розрізняє «дерево порожнє»
> (git-константа `4b825dc642cb6eb9a060e54bf8d69288fbee4904`, на яку GitHub віддає **404**)
> і «об'єкта нема». Плагін уміє САМ створити цей стан — «видалив усі файли → синк» шле
> deletion-entries, і результівне дерево порожнє, — тож наступний drain на цьому вмирав.
...
**⚡ ЗВІДКИ БЕРЕТЬСЯ ВІДПОВІДЬ — спершу з дерева, і лише потім із мережі (2026-09-01,
зміряно на живому bootstrap власника).** Шар 2 лишається обов'язковим і незмінним за змістом;
змінюється тільки ТРАНСПОРТ. Привід — вимір: холодний старт на 63 МБ vault дав **283 запити й
90.1 с у мережі, з яких 255 запитів і 78.1 с — саме ці per-path HEAD**. На холодному старті в
батч потрапляє КОЖЕН локальний файл (baseline-ів нема), тож Шар 2 питав мережу по одному разу
на файл — про той самий `{sha,size}`, який Шар 1 уже привіз одним рекурсивним читанням дерева
запитом раніше.

Правило: якщо discovery читав ПОВНЕ дерево (холодний старт або будь-який fallback), він віддає
разом зі списком змін **знімок** `{atCommit, paths: Map<path,{sha,size}>}`. Шар 2 бере відповідь
із нього, **якщо і тільки якщо `atCommit === head_hash` поточного кроку**; інакше — мережа.

Чому це не послаблення гарантії:
- `atCommit` — це SHA **коміта**, не назва гілки. Дерево коміта незмінне, тож `<path>@<sha>`
  фізично не може відповісти інакше. Знімок не «кеш відповіді», він і **є** відповідь.
- Знімок лишається **незалежним** читанням рефа: холодний шлях Шару 1 порівнює baseline-и
  з деревом, а Шар 2 — переконання журналу (`tracked.remote.sha`) з тим самим деревом. Різні
  переконання, одне джерело істини — блайндспот-перевірка й далі щось перевіряє.
- Відсутність шляху в знімку = 404 від Contents API. Ця рівність тримається ЛИШЕ на повноті
  дерева: обрізане (`truncated`) дерево знімка не породжує взагалі (§II.12 кидає `TreeTruncatedError`).
- Знімок містить **усі** блоби, не лише syncable: шлях може бути поза scope зараз, але існувати
  на сервері, і відфільтрований знімок показав би наявний файл як видалений.
- ⚠️ Умова — саме `atCommit === head_hash`, а не «знімок є». `head_hash` **котиться** після
  кожного push-у батчу, і мапа, що відповідає за чужий коміт, — це рівно той мовчазний clobber,
  проти якого Шар 2 і існує. Невідомий коміт → мережа, без винятків.
- Межа, зафіксована тестом (P.29a-bis): коли remote НЕ рухався, §III взагалі не кличе discovery
  (`head_hash === base_hash`), тож знімка нема й Шар 2 платить за шлях. Це дешево — у батчі лише
  те, що юзер щойно змінив.

**Результат виміру на тому самому vault:** `contents-head` **255 → 0**, увесь прохід
**283 → 16 запитів, 90.1 с → 5.3 с у мережі, ~87 с → 7.1 с wall-clock**. Тести: P.29a…P.29e +
P.29a-bis (`tests/sync2/drain.test.ts`), RED-перевірені — вимкнений знімок валить 3, прибрана
перевірка коміта валить рівно P.29c.

**Що НЕ чіпаємо (свідомо):** `getCommitInfoForPath` (mtime для §II.1 п.3.b.e — джерела дешевшого
немає, і викликів одиниці) і live-читання голів гілок (§II.7 вимагає їх свіжими). Спроба дістати
`tree_sha` з відповіді `GET /git/trees/<commit>` ПЕРЕВІРЕНА й відкинута: GitHub повертає в полі
`sha` той самий commit-sha, який ми передали, а не sha дерева — джерела там немає.
...
### II.13.1 Шар 2 обнуляє `remote.mtime` — і хтось мусить його повернути (польова знахідка 2026-09-23)

Шар 2 замінює віддалену половину **цілком** і свідомо скидає `mtime`/`deviceLabel`:
виправлений вміст має невідомі дату й авторство, і стара дата не повинна просочитись
у назву sibling-файлу. Це правильно.

Але §II.1 п.3.b.e — «новіший перемагає» — читає саме цей `mtime`. А `_diff3` вирішує
3.b.e **всередині себе** й мережі не має. Тому після виправлення Шаром 2 у тайбрейк
приходить `null`, і null-guard у `pickNewestForObsidian` віддає перемогу віддаленому
**завжди**. Задокументоване «новіший перемагає» не виконується взагалі.

Це вже знаходили — gate finding **E4** — але дозапит додали лише в гілку
`plugin-dispatch`. Звичайні `.obsidian/`-шляхи його не отримали, і дефект прожив до
польового логу: свіжо-відновлений `.obsidian/.gitignore` (секунди від роду) програв
віддаленій копії, писаній попередньою версією плагіна, а виправлення повернулось
окремим комітом наступним проходом.

**Виправлення — одне джерело умови, один хелпер дозапиту:**

```
# diff3.ts — умова 3.b.e названа один раз і експортована
needsObsidianMtimeTiebreak(tracked, local) -> bool     # дзеркалить ранні return-и _diff3

# drain.ts — ОДИН лінивий дозапит, спільний для обох сайтів
fillRemoteMtime(path, tracked)    # no-op, якщо mtime уже відомий або репо порожнє

if needsObsidianMtimeTiebreak(tracked, local): fillRemoteMtime(...)
verdict = _diff3(...)
if verdict is plugin-dispatch:     fillRemoteMtime(...)   # той самий хелпер
```

Ціна лишається нульовою на щасливому шляху: однобічна зміна `.obsidian/`-файлу йде
через 3.b.1.a/b і **не платить нічого** — це важливо, бо вмикання «Sync config» кладе
десятки таких файлів в один батч.

**Чому це не ловилось мутаційною пробою.** Кожен рядок робив те, що написано в його
власному коментарі; дефект жив у ШВІ між двома шарами — один нулює, інший читає, а
домовленості про те, хто повертає, не було в жодному з них. Тому пін тут не рядковий,
а **вичерпна матриця чутливості** (`diff3.test.ts`, «A1 🔑 exhaustive»): для всього
простору входів `_diff3` проганяється двічі зі **зміненими місцями** mtime, і
перевіряється тотожність «вердикт залежить від mtime ⟺ предикат каже, що залежить».
Розходження предиката з реальністю — у будь-який бік — валить її.

Побічно та ж проба показала, що **сам фікс E4 теж не мав тесту**: його видалення
лишало сюїту зеленою. Закрито A1 п.25.

### II.13.2 Знімок беремо САМІ, а не чекаємо на випадковий (рішення власника 2026-09-25)

Правило вище — «Шар 2 бере відповідь зі знімка, якщо `atCommit === head_hash`, інакше
мережа» — лишає джерело відповіді **випадковим**. Знімок з'являється тільки тоді, коли
його для СВОЇХ потреб прочитав Шар 1, а він читає повне дерево лише на холодному старті
або у fallback-ах. У звичайному житті Шар 1 бере `compare()`, і той знімка не дає —
«що змінилось» не містить «що існує».

**Стик, який від цього болить:** локально багато файлів, а віддалене майже не рухалось.
Тоді Шар 1 іде дешевим шляхом (знімка нема), а Шар 2 мусить питати про десятки шляхів —
і йому лишився тільки per-path транспорт.

Це не рідкість «майже ніколи», це рідкість «на певні події»: **прийняття в синхронізацію**
(вмикання `Sync config`, впуск нового dot-простору) і холодний старт. Тобто саме ті
моменти, коли користувач уперше пробує плагін.

**Вимір (польовий лог власника 2026-09-23):** 46 файлів у батчі, 41 per-path запит,
**≈15 с із 23 с** усього drain'у. Раніше зміряний гірший випадок (§II.13 вище): 255
запитів, **78 с із 90 с**.

#### Рішення

Коли знімка немає (або він від ЧУЖОГО коміта), а в батчі **≥ 4 записів** — прочитати
дерево на поточній голові **самим**, одним запитом, і покласти його в те саме
`remoteTree`. Логіка Шару 2 при цьому не змінюється **жодним рядком** — змінюється лише
спосіб придбання джерела.

```
ensureRemoteTreeSnapshot(batchSize):          # ліниво, на ПЕРШОМУ промаху
    if знімок уже є і atCommit == head_hash:  return          # нічого робити
    if batchSize < 4:                         return          # дешевше спитати по одному
    if для цієї голови вже пробували:         return          # не питати на кожен файл
    files, truncated = getRepoTree(head_hash)
    if truncated:                             return          # ⚠️ див. нижче
    remoteTree = { atCommit: head_hash, paths: files }
```

**Поріг = 4 (власник, 2026-09-25).** Нижче цього дешевше спитати по одному: дерево
може важити мегабайти, а три запити — це три поїздки. Число підкручуване, але воно
**рішення, а не константа з повітря**.

#### Чому це не послаблює гарантію

Ті самі три аргументи, що вже стоять у §II.13 для «випадкового» знімка, — вони не
залежать від того, ХТО зробив запит:

- `atCommit` — SHA **коміта**, дерево коміта незмінне, тож `<path>@<sha>` фізично не може
  відповісти інакше. Знімок не кеш відповіді, він **і є** відповідь;
- читання лишається **незалежним**: Шар 1 звіряє baseline-и з деревом, Шар 2 —
  переконання журналу з тим самим деревом. Різні входи, одне джерело;
- знімок містить **усі** блоби, не лише syncable (`getRepoTree` не фільтрує) — інакше
  наявний файл поза scope показався б видаленим.

Умова `atCommit === head_hash` лишається дослівною: голова котиться після кожного push-у
батчу, і мапа за чужий коміт — це рівно той тихий clobber, проти якого Шар 2 і існує.

#### ⚠️ `truncated` — єдина пастка, і вона фатальна

Уся рівність «**відсутній у знімку = 404 = файла на сервері немає**» тримається на
ПОВНОТІ дерева. GitHub ріже відповідь на 100 000 записів **або** 7 МБ (SPIKE-TREES-LIMIT
§3) і ставить `truncated`. На обрізаному дереві «немає в списку» означає «не влізло» — а
ми прочитали б це як «видалено», запушили б поверх і **знищили б чуже мовчки**. Тобто
рівно той наслідок, проти якого весь Шар 2 і побудований.

Тому: **`truncated: true` → знімка НЕ будуємо, відкочуємось на per-path.** Повільно й
правильно замість швидко й хибно. Позначка «для цієї голови вже пробували» при цьому
лишається — інакше обрізане дерево тягнулося б наново на КОЖЕН файл батчу.

#### Транспорт — уже наявний і зміряний

`GET /git/trees/{commit-sha}?recursive=1` (`client.getRepoTree`), той самий, яким
користується холодний старт. Зміряно на живому API 2026-08-30 (SPIKE-TREES-LIMIT §3):
**20 002 записи → 598 мс, 5.6 МБ, 278.5 Б на запис.** Для порівняння: 41 per-path запит
≈ 15 с, 255 ≈ 78 с. Різниця не в об'ємі, а в **латентності**: per-path упирається в
кількість поїздок (~350 мс кожна), дерево — в одну.

Стеля відома й обмежена: найгірший випадок цього запиту — одна передача до 7 МБ. У
per-path стелі немає взагалі — 1000 файлів це 1000 поїздок.

⚠️ 598 мс зміряно з десктопу. Мобільне радіо зробить 5.6 МБ повільнішими — але воно
зробить повільнішими й 41 поїздку, тож порівняння не перевертається. Точне мобільне
число — окремий вимір ПІСЛЯ впровадження, тим самим логом.

#### ⚠️ Стеля виграшу — ~20–25k файлів, і деградація там ОБРИВНА

Питання власника 2026-09-25: «а якщо в репозиторії 20 000 файлів?». Відповідь
неприємна й має бути тут, а не виводитись заново.

Дерево впирається не в кількість записів (100 000), а в **7 МБ**. SPIKE-TREES-LIMIT §4:
на 20k-vault ми вже на **~80% від 7 МБ**, тож справжня стеля ≈ **25k файлів**. І це для
шляхів ~40 символів латиницею — реальні шляхи довші, а кирилиця в UTF-8 коштує ~2 байти
на символ, тож для цього власника стеля настане **раніше**.

Що на стелі:

- discovery (холодний шлях) **кидає** `TreeTruncatedError` — плагін уже офіційно визнає
  себе непрацездатним на такому масштабі;
- Шар 2 відкочується на per-path — і при великій пачці це 20 000 × ~0.35 с ≈ **дві
  години**.

Тобто деградація не плавна, а обривна. Зміна нічого не **погіршила** (до неї в цьому
сценарії per-path був завжди), але й виграш має стелю, і це треба знати: **він працює до
~20–25k файлів, далі зникає.**

**РІШЕННЯ ВЛАСНИКА 2026-09-25: стеля лишається стелею ПЛАГІНА.** Обхід дерева по
каталогах (SPIKE §4 — один `GET /git/trees/{sha}` на рівень, без 7 МБ-стелі) **НЕ
будуємо**. Сховища понад ~20k файлів плагін не підтримує; кому треба більше — інший
інструмент.

Наслідки, які з цього випливають і вже зроблені:

- межа названа в README як явна відмова в підтримці, поруч із рештою «чого плагін
  свідомо не робить» — із числом, причиною (7 МБ) і поправкою на не-ASCII шляхи;
- `TreeTruncatedError` більше не інженерний жаргон: повідомлення **користувацьке**, бо
  воно доходить до Notice. Каже що сталось, що нічого не змінено і що робити;
- Шар 2 при обрізанні **лишає** відкат на per-path, а не падає слідом. Це навмисно:
  сховище трохи за стелею у щоденному циклі працює справно (пачка < 4 → дерево не
  читається взагалі), і ламати робочий випадок заради симетрії — гірше, ніж мати дві
  різні поведінки. Ламається рівно те, що не може не зламатись: холодний старт і великі
  пачки;
- попередження про обрізання друкує `perPathRequests`, щоб ціна була видима в лозі, а не
  виглядала як зависання.

Розгорнутий розбір механізму обходу (властивість «sha каталогу = хеш усього піддерева»,
відсікання незмінених гілок, зміряні числа) — у діалозі 2026-09-25; сюди не переноситься,
бо рішення «не будуємо».

#### Надлишковість `compare()` + дерево в одному прогоні — знана, ціна 1 запит

Коли віддалене рухалось І в пачці ≥ 4 записів, ми робимо обидва запити, хоча одне
дерево відповіло б на обидва питання. Ціна — **один `compare()`**, і вона НЕ росте з
розміром репо: відповідь `compare()` пропорційна різниці й обмежена 300 записами.

Чому не прибираємо:

1. discovery виконується **до** `claimBatch()`, тож у момент вибору «compare чи дерево»
   розмір пачки ще невідомий — виправлення означає перестановку порядку в drain'і;
2. це не той самий транспорт, а **інше порівняння**: `compare(base, head)` питає «що
   змінилось на сервері між комітами», а дерево+baseline-и — «чим сервер відрізняється
   від нашої памʼяті». Підміна змінила б семантику Шару 1, не лише його вартість;
3. «робити дерево, коли змін < 4» неможливо в принципі: `compare()` **і є** тим
   запитом, який повідомляє кількість змін.

У найдорожчому польовому сценарії цієї надлишковості взагалі немає: там `head == base`,
discovery не викликається, і читання дерева — єдиний запит.

#### Що це НЕ робить

Не звужує Шар 2 за випадками. Розбір 2026-09-25 показав, що при `base ≠ null` і явно
названому discovery шляху перевірка **майже** надлишкова (питаємо той самий незмінний
коміт про шлях, на який нам щойно з нього ж відповіли) — але:

1. виграшу це не дає: у дорогому сценарії таких шляхів нема (усі 41 корекції в логу
   мали `expected = 000…0`, тобто «уявлення не було взагалі»);
2. це зняло б хай і слабку, але перевірку з механізму класу «тихе знищення даних».

Якщо колись знадобиться — окреме рішення з окремим виміром, не побічним ефектом цього.
...
## II.16 Прогрес для користувача — два лічильники, один таймер (рішення власника 2026-09-25)

Старий двигун показував «Sync done» і рахував файли лінійно. Новий цього не може, і
причина структурна: **pull і push перемежовуються**. Один прохід по батчу може
відправити файл, притягнути інший і знову відправити — послідовності «спершу все
завантажили, потім усе відправили» не існує. Тому й повідомлення виду «отримуємо N з M,
а потім відправляємо K з L» побудувати неможливо.

### Модель: ДВА незалежні лічильники, обидва видимі одночасно

| лічильник | M береться з | коли росте M |
|---|---|---|
| **push** | `claimed.meta.entries.length` | += розмір КОЖНОГО нового взятого батчу (3 → 3+5 = 8) |
| **pull** | `remoteFiles.length` із відповіді discovery | += при КОЖНОМУ новому виклику discovery (зокрема після 422-рестарту) |

(Докладніше про ДЕ саме інкрементується pull і чому push поводиться інакше — нижче.)

⚠️ **M зростає на льоту — це не дефект, а властивість.** Drain не знає наперед, скільки
батчів у черзі й скільки разів переспитає сервер. Напис «3 з 5», що стає «4 з 8», чесно
відображає те, що робота побільшала — на відміну від фальшивого «3 з 8», яке вдавало б,
ніби ми знали це з початку.

**Повідомлення показуємо ПЕРЕД операцією, не після.** «Downloading 2 of 10» означає
«зараз беруся за другий» — бо користувачеві потрібен сигнал життя саме на час очікування.
Одне логічне «завантаження» може фізично означати ДВА запити (base + remote) — лічильник
рахує ФАЙЛИ, не запити.

### ДЕ саме інкрементується pull (власник, 2026-09-25)

Не «коли файл завантажено», а **коли drain береться за цей шлях** — до будь-якої роботи
над ним. Обґрунтування власника: завантаження BASE і REMOTE живе всередині `_diff3`, тож
рахувати треба перед ним; а якщо перемагає локальна версія, віддалена просто
позначається обробленою і не тягнеться — «та хто це рахує».

Уточнення, знайдене при звірці з кодом: інкремент стоїть **на крок раніше за `_diff3`**,
бо безпосередньо перед ним у циклі по батчу є короткий вихід

```ts
if (tracked.remote.sha !== null && tracked.remote.sha === local.sha) { … continue; }
```

— випадок «на сервері змінилось рівно на те, що вже лежить локально». Віддалена зміна є,
discovery порахував її в M, але до `_diff3` вона не дійде НІКОЛИ, і лічильник застряг би
на «9 з 10». Позиція «коли беремось за шлях» покриває і цей випадок, тож **кожна віддалена
зміна рахується рівно один раз, і лічильник чесно доходить до M**.

⚠️ `_diff3` для звичайних шляхів викликається у ДВОХ місцях — цикл по батчу і Vault-step
— і шлях, присутній і там, і там, пройшов би обидва. Тому інкремент захищений набором
«вже пораховано»: один шлях = одна одиниця за прогін.

**Рішення власника — два правила лічильників і одне про форму:**

1. **PUSH при 422-рестарті ВІДКОЧУЄТЬСЯ, не накручується.** Рестарт повторює той самий
   батч; числа повертаються до значень ДО нього і рахуються заново («4 з 8» → «3 з 3» →
   далі вгору). Аргумент власника, і він сильніший за монотонність: людина, що змінила
   три файли й побачила шість, подумає, що на сервер пішло щось зайве. **Страх втрати
   контролю гірший за незручність відкату.**
2. **PULL при рестарті ЗРОСТАЄ** — discovery питає сервер наново, нові значення
   додаються до M.

   ⚠️ **Асиметрія свідома, і ось чому:** push — це файли, які змінив САМ користувач, їхню
   кількість він знає, і завищення читається як «плагін робить щось не те». Pull — це
   новини З СЕРВЕРА, їхня кількість йому невідома наперед, і зростання читається
   природно: «прилетіло ще». Одне й те саме число означає для людини різні речі залежно
   від того, чиє воно.
3. **Рядки з нулями НЕ показуємо.** «Downloading 0 of 0» на чистому push-і — шум, який
   щоразу привертає увагу ні до чого. Те саме для конфліктів: рядок зʼявляється лише
   коли їх > 0.

### Коли показувати: ЄДИНА умова — час

Ані розмір батчу, ані кількість змін на сервері не вмикають прогрес. **Тільки тривалість:
синхронізація триває довше 2 с → показуємо з поточного стану.**

Чому саме так (власник, 2026-09-25, після розбору розмірного варіанта): скарга була про
**час** («паузи понад 3-4 секунди обтяжливі»), а розмір — лише проксі для часу, і на
поганому каналі проксі бреше. Два файли по повільному радіо мовчать довше за сорок по
Wi-Fi. Одна умова замість чотирьох, і вона міряє саме те, на що скаржились.

Наслідок, прийнятий свідомо: користувач часто побачить не «1 з 10», а одразу «3 з 10» —
бо перші два вже пройшли за ті 2 секунди. Це правильно: прогрес показує, **де ми зараз**,
а не переповідає те, що вже минуло.

Таймер зводиться від початку ВСІЄЇ синхронізації, не від drain-у: користувач чекає з
моменту кліку, і commit-прохід на 1.5 с + drain на 1 с — це 2.5 с мовчання, хоча жодна з
фаз порогу не перетнула. Доки лічильників ще нема, повідомлення показує лише заголовок.

### Форма

```
Syncing with GitHub
Downloading 2 of 10          ← лише коли pullTotal > 0
Uploading 3 of 5             ← лише коли pushTotal > 0
⚠ 2 files need resolving     ← лише коли конфліктів > 0
```

Без слів pull/push: для людини, що не знає git, вони порожні.

### ОДНЕ повідомлення на всю операцію (власник, 2026-09-26, після знімків)

Не «тост на подію», а **один `Notice`, який змінює текст**:

```
Nothing to commit  →  (прогрес, якщо затягнулось)  →  Sync done — 6 sent
└──────────────────── те саме повідомлення ───────────────────┘
```

Наприкінці воно висить ще ~1 с і зникає.

**Чому не окремі тости.** Вони **складаються в стос**, і Obsidian вирівнює стос по
ширині: коротший текст сидить у коробці, розтягнутій під ширший. А коли верхній тост
згасає, нижній стискається до своєї ширини **і стрибає вгору** на звільнене місце. У
полі це прочиталось як «два різні повідомлення, одне з них із пробілами в кінці» —
насправді це було ОДНЕ повідомлення, що змінило ширину й позицію. Знімки:
`docs/tasks/imgs/bug-sync0.png` (стос) і `bug-sync1.png` (після згасання верхнього).

**Одне повідомлення не може скластися в стос саме з собою** — ось і вся причина.

⚠️ Старий двигун цю граблю вже проходив: у його коді є коментар «Both used to surface
their own brief toast, but the user reported seeing them stack… **Single-notice UX**».
THE SWITCH повернув окремі тости, і на них наступили вдруге. Записано тут, щоб не було
втретє.

**Межі моделі:**

- **commit БЕЗ drain-у** (кнопка [Commit]) лишається коротким тостом, який гасне сам:
  за ним нічого не йде, тож нічого й не замінить його текст;
- **прогрес не перемальовує повідомлення, доки не минули 2 с** — інакше він затер би
  щойно показаний результат commit-у й зруйнував саму послідовність, заради якої все це;
- ⚠️ `duration: 0` не гасне сам. Зняти повідомлення мусить КОЖЕН вихід — підсумок,
  помилка, скасування **і `onunload`**: якщо плагін вивантажиться посеред синку (вимкнення
  або перезавантаження при самооновленні), повідомлення переживе плагін, який його створив.

⚠️ **Notice з `duration: 0` не зникає сам.** Його `hide()` мусить стояти у `finally`
кожного виходу — успіх, помилка, скасування, викинутий виняток. Інакше він висітиме до
перезапуску Obsidian, і це гірше за відсутність прогресу взагалі.

### Статус-бар — не чіпаємо, крім одного дефекту

Статус-бар лишається як є (`GitHub (↑ N | M ⁇)`): глибина черги й конфлікти, без
прогресу. Але звірка 2026-09-25 знайшла, що `fireQueueDepth()` викликається **один раз
після всього drain-у**, а не після кожного знятого батчу — тож при чотирьох батчах `↑ 4`
висить незмінним і стрибає в нуль наприкінці. Глибина мусить спадати по батчах.

---

## II.17 Скасування — ЧОТИРИ межі, і одна з них навмисно без перевірки

Скасування існує з 2.0.2-beta (`cancelRequested`), але звірка 2026-09-26 показала, що
перевірок було **дві з чотирьох**:

| місце | що там відбувається | перевірка |
|---|---|---|
| `while` по батчах | межа БАТЧУ | ✅ була |
| `for` по записах батчу | межа ФАЙЛУ | ✅ була |
| **перед push-ом батчу** | flush → commit → ref, ~3-4 с мережі | 🔴 **не було — додано** |
| **Vault-step** | запис кожного притягнутого файлу | 🔴 **не було — додано** |
| **перед FINALIZE** | compare → merge → ref → delete | 🔴 **не було — додано** |
| епілог (baseline-и, якір, чистка) | — | ⛔ **навмисно НЕ додаємо** |

**Польовий звіт 2026-09-26, який і показав головну дірку:** користувач натискав
[Cancel sync] і **жодного разу** не бачив підтвердження. Причина — цикл по файлах
ЛОКАЛЬНИЙ і добігає за мілісекунди, а чекає користувач на тому, що йде ПІСЛЯ нього:
`POST tree` 1.3 с + `commit` 0.6 с + `PATCH ref` 1.2 с. Саме туди й потрапляв клік, і
там не було чого його зловити. Перевірки на межі файлу недостатньо, коли робота між
двома такими межами триває довше за весь цикл.

Безпека push-межі — той самий аргумент D.16: жодного рефа цього батчу ще не зрушено,
каталог батчу не видалено, журнал тримає ОСТАННІЙ ЗАВЕРШЕНИЙ батч. Проміжні flush-і
могли лишити на GitHub dangling-дерева — обʼєкти без рефа, які GitHub прибирає сам,
рівно як після 422-рестарту.

FINALIZE пропустити теж безкоштовно: §II.14 спершу перевіряє досяжність, тож merge
ідемпотентний, і наступний drain знайде ту саму гілку.

**Чому Vault-step був найгіршим місцем для цієї дірки.** Саме він пише в сховище кожен
притягнутий файл і дозавантажує потрібні блоби — тобто це фаза, де користувач і чекає
при великому pull-і. Натиснута кнопка [Cancel sync] не робила **нічого**, доки вся фаза
не добігала.

**Чому вихід звідти безпечний — і це не нове припущення.** У тому ж циклі вже стоїть
мережевий abort із рішенням власника: *«abort, never per-file skip — the journal stays,
the next drain repeats the WHOLE Vault-step»*. Тобто стан «вийшли посеред Vault-step»
уже існує, вже описаний і вже обслуговується. Скасування лише **приєднується до
перевіреного шляху**, а не винаходить власний. Журнал чистить тільки епілог (крок 4),
тож наступний drain відновиться й доробить.

**Чому епілог перевірки НЕ отримує (рішення 2026-09-26).** Епілог — це «зафіксувати те,
що вже зроблено»: baseline-и, hot-якір, смерть журналу, sweep. Перервати його означає
лишити виконану роботу незафіксованою. Він короткий і майже без мережі (один `getCommit`,
та й той лише коли голова не рухалась — §II.7.1), тож затримка від його добігання
непомітна, а користь від переривання відʼємна.

### Скасований синк НЕ звітує про успіх

Скасований drain повертається **нормально** — це не помилка. Тож підсумкове
повідомлення §II.16 радісно сказало б «Sync done» одразу після того, як користувач
натиснув [Cancel sync].

`onSyncCompleted` тому несе прапорець `ok: false` — не звітувати про успіх (помилка АБО
скасування). Але саме **підтвердження** живе не там, а на переході drain-у в `idle`
(слухач статусу в `main.ts`), і це не дрібниця розміщення:

- `onSyncCompleted` спрацьовує лише для `syncAll`/`syncFile`. **Фоновий drain** (тік
  інтервалу, watchdog) його не викликає взагалі — скасувавши такий, користувач не
  побачив би нічого НІКОЛИ;
- перехід у `idle` стається для КОЖНОГО шляху, який можна скасувати.

**Тост «cancellation requested» прибрано** (власник, 2026-09-26): користувач щойно
натиснув кнопку з написом [Cancel sync], і повторювати йому його ж клік — шум.
Повідомляти треба те, чого він НЕ бачить: що drain справді зупинився. Лишається одне
повідомлення — **«Sync canceled»**, у момент фактичної зупинки.

### ⚠️ Відома вада, яку скасування УСПАДКОВУЄ (не створює)

Baseline-и пишуться **гуртом в епілозі**, не по файлах. Тож вихід посеред Vault-step
лишає стан «файли записані, baseline-и ще старі». Наслідок: наступний commit-прохід
вважає притягнуті файли локально зміненими й кладе їх у батч — а drain їх одразу
закорочує (локальний sha == віддалений → push не відбувається).

Ціна: **один змарнований цикл «коміт → drain у нікуди»**. Втрати даних немає. І це
поведінка наявного мережевого aborta — скасування її не привносить. Якщо колись
захочеться прибрати: baseline-и мали б писатись по ходу Vault-step, а не гуртом; це
окреме рішення з власним виміром.
...
> 🔒 **ІНВАРІАНТ ПАРИ (доданий 2026-09-20 після дефекту W1, §VIII D):**
> **прапорець `isManualConflict`, який можна прочитати з журналу, означає, що
> durable-запис у `conflicts.json` УЖЕ існує.** Тому на межі «BATCH ОБРОБЛЕНО!»
> спершу йде `conflictStore.save(conflicts)`, і лише потім `journal.persist(state)`.
>
> Чому саме такий порядок — з чотирьох станів краху деструктивний рівно один:
>
> | журнал | `conflicts.json` | наслідок |
> |---|---|---|
> | прапорець | **запису нема** | 🔴 RECONCILE читає порожній скан як «розв'язано ззовні» → гасить прапорець → FINALIZE зливає й видаляє гілку → наступний батч бере правило 4.4 і затирає theirs на main. Тихо, клас G9 |
> | нема | нема | ✅ `_diff3` виводить конфлікт заново; жива перевірка в `shouldPushToConflictBranch` тримає push у гілку ідемпотентним |
> | нема | запис є | ✅ seeding (J.3) піднімає прапорець назад |
> | прапорець | запис є | ✅ RECONCILE коректний |
>
> Запис сховища ПЕРЕД журналом робить єдиним досяжним проміжним станом рядок 3.
>
> ⚠️ **Пара — рівно в одному місці, і це навмисно.** Мінт імені гілки персистить
> ЧИСТИЙ стан (D.16) і пари не отримує; виходи по 422-CAP і cancel не персистять
> нічого взагалі (запінено ассерцією в I6-тесті); персисти всередині FINALIZE
> працюють за умови `len(conflicts)==0`. Конфлікт, народжений на Vault-step,
> пари теж не потребує: **після Vault-step журнал не персистять ніде**, тож його
> прапорець не переживає прогін — на наступному запуску конфлікт просто виводиться
> заново.
>
> 🔑 **Чому дефект був неочевидним:** журнал НЕСЕ `conflicts` у тому ж блоці
> (`TrackedFiles + conflicts + conflictBranchName`), тобто крах-протокол мав усі
> дані — але блок відновлення свідомо викидає цю копію («the journal's bundled
> copy is SUPERSEDED», `drain.ts`), бо авторитетним для siblings-половини є скан.
> Відновлювати конфлікти з журналу натомість — розглянуто й ВІДХИЛЕНО: панель і
> лічильник конфліктів читають СХОВИЩЕ, тож між крахом і наступним drain-ом
> користувач не бачив би конфлікту зовсім.
...
## IX. Звірка з кодом — 2026-09-23 (`cf3c681`, 2036 юніт-тестів зелені)

Аудит «вимога проти коду», а не перечитування. Одиниця — **нумерована вимога**
(секція §II.x, рядок таблиці IV, §V, §VI), не рядок псевдокоду: змістовних тверджень
у документі кількасот, рядків §III — ~2540, і звіряти треба перші.

**Що це НЕ:** це не оцінка покриття §VIII (її робив аудит 2026-09-20 по категоріях) і
не перевірка, що тести не вакуумні. Тут питання одне: **чи код робить те, що написано**.

**Пройдено:** прохід 1 (механічне трасування) повністю, прохід 2 (семантична перевірка)
на несучій підмножині, прохід 3 (мутаційні проби) — дев'ять проб, §IX.3. Результат: одна
знахідка — фікс W1 не був запінений жодним тестом.

### IX.1 Вердикти

| вимога | вердикт | де в коді |
|---|---|---|
| §II.1 таблиця правил `_diff3` | ✅ MATCHES (структурно) | `diff3.ts` — кожне нумероване правило має підписану гілку: 2.a/2.b, 3.a, 3.b.1.a/b, 3.b.2.a/b, 3.b.\*.e, 4.1.a-d, 4.2, 4.3, 4.4, 4.5.a/b, 4.6.a/b, 4.7 |
| §II.2-II.5 сценарії drain | ✅ MATCHES | `drain.ts:22,29,554` |
| §II.6 manual-conflict машинерія | ✅ MATCHES | 13 сайтів, `drain.ts` + `drain-journal.ts` |
| §II.7 ім'я ПЕРЕД мережею, голова — жива | ✅ MATCHES | `ensureConflictBranch()` у `drain.ts` (§II.7.1 — пара «мінт+персист → мережа» переїхала туди цілою, 2026-09-23); `drain-journal.ts:12` (голови НЕ персистяться). Пін G.5 сформульований як інваріант, тому переїзд пережив без правок — ре-проба на новому місці: обидві мутації (персист знято / персист ПІСЛЯ мережі) валять саме його |
| §II.8 claim-протокол R3b | ✅ MATCHES | `get-batch.ts` (+ `.attempted` у `batch-metafile.ts`) |
| §II.9 hash-on-load | ✅ MATCHES | `sync-store.ts`, `diff3.ts:314` |
| §II.10 один retry-хелпер | ✅ MATCHES | `retry-network.ts` |
| §II.11 mark-транзакція STEP3 | ✅ MATCHES | `drain.ts:386,1315,1411`; `conflict-siblings.ts:135` |
| §II.12 гібридний discovery | ✅ MATCHES | `discovery.ts`, `client.ts:852`, `errors.ts:187` |
| **§II.13 Шар 2 ПЕРЕД short-circuit** | ✅ **MATCHES — звірено порядком** | Шар 2 `drain.ts:810`, short-circuit `:899`. Guard на `headHash===null` і правило «знімок лише при `atCommit === head_hash`» — на місці |
| §II.14 reachability-merge | ✅ MATCHES | `drain.ts:165,1163,1227,1239` |
| §II.15 акумулятор + `uploadedBlobs` | ✅ MATCHES | `tree-accumulator.ts`, `client.ts:377` |
| **IV.2 / W1 порядок на межі батчу** | ✅ **MATCHES — звірено** | `drain.ts:1136-1158`: сховище ПЕРЕД журналом, із розписаною таблицею чотирьох станів краху |
| **IV.3 маркер `processed_batch`** | ⬜ **НЕ РЕАЛІЗОВАНО** | у коді відсутній. Документ сам називає його **оптимізацією, не умовою коректності**, тож розходження легітимне — але ціна названа: крах між «журнал записано» і «каталог видалено» дає повний redo з мережевими читаннями замість дешевого пропуску |
| §V ping-pong, max-seq слот НЕ пишемо | ✅ MATCHES | `hot-metadata.ts:11-13` — інваріант явний. ⚠️ Код посилається на METAFILE §2.1, не на §V: одне правило, два документи |
| §VI.1 межа 1 (між батчами не паралелимо) | ✅ MATCHES за побудовою | послідовний `for…of` по батчах |
| §VI.2 per-file ПОСЛІДОВНО, не `Promise.all` | ✅ MATCHES | у `drain.ts` немає жодного `Promise.all` |
| §VII відкриті питання | ✅ усі 5 позначені ВИРІШЕНО/ПІДТВЕРДЖЕНО; тихо-відкритих нема | — |

> ⚠️ **Що ці ✅ означають, а що ні.** Кожна клітинка тут — твердження «КОД відповідає
> ДОКУМЕНТУ», звірене читанням. Вона нічого не каже про те, чи щось утримає цю
> відповідність при наступному рефакторі. Проходи 3 і 4 (IX.3, IX.3.1) міряли саме
> це — і знайшли вісім несучих рядків, які були правильні й не трималися нічим
> (W1 + сім у §II.6 / sweep / §III). Жоден із них не спростував рядка цієї таблиці:
> код скрізь був правий. Читати ці ✅ як «покрито тестами» — помилка, і саме вона
> 2026-09-20 дала завищені клітинки покриття.

### IX.2 Розходження, де правий КОД (документ треба патчити)

1. **§I.1 описує до-D1 модель dot-простору.** Текст: «Metadata.files зберігає …
   включно з тими, які знаходяться в дозволених "dot"-каталогах (**тих, яких нема
   в `.gitignore`**)». Після DOT-FILES Кроку B2 дозвіл дає не «відсутність у
   `.gitignore`», а **opt-in-набір** (`readRootGitignore`: названі dot-ФАЙЛИ +
   анкеровані walk-targets), і членство в ньому — умова D7, без якої шлях не
   синкається взагалі. Формулювання §I.1 треба замінити посиланням на
   [`DOT-FILES §4.2`](./SYNC2-DOT-FILES-REFACTOR.md) / [`SYNC2.md §13`](../SYNC2.md).
2. **Discovery тепер має власний lifecycle, якого документ не знає:**
   `ChangeDetector.beginScan()/endScan()` фіксує scope на всю операцію (DOT-FILES §5),
   а `isSyncable` кидає, якщо scope не встановлено. §II.12/§III цього не згадують.
3. **Порожнє дерево більше не помилка.** `getRepoTree` розрізняє «дерево порожнє»
   (git-константа `4b825dc6…`, на яку GitHub віддає 404) і «об'єкта нема». Документ
   писався до цього фіксу.

Жодне з трьох не є дефектом коду — це борг документа.

### IX.3 Прохід 3 — мутаційні проби (2026-09-23)

Метод — той самий, що 2026-09-20 знайшов завищені оцінки покриття: **зламати рядок,
який тест нібито пінить, і подивитись, чи впаде хоч щось.** Назва сценарію в назві
тесту не доводить нічого; падіння — доводить.

Девʼять проб на несучих інваріантах, по одній мутації за раз, дерево щоразу
відновлювалось:

| проба | мутація | падінь | вердикт |
|---|---|---|---|
| 4.5.a null-як-база | `local` → `remote` | 13 | ✅ запінено |
| 4.5.b null-як-база | `remote` → `local` | 1 | ✅ запінено (тонко) |
| 4.6.b асиметрія | `manual-conflict` → `remote` | 2 | ✅ запінено |
| 3.b.\*.e mtime | `pickNewest…` → завжди `local` | 4 | ✅ запінено |
| §II.13 `atCommit === head_hash` | умову знято | 1 | ✅ запінено (рівно P.29c, як і обіцяв документ) |
| §V слот ping-pong | пишемо MAX-seq слот | 8 | ✅ запінено |
| §II.7 ім'я перед мережею | `persist` прибрано | 3 | ✅ запінено |
| §II.9 hash-on-load | довіряємо імені замість хеша | 5 | ✅ запінено |
| **§IV.2 / W1 порядок** | **сховище ↔ журнал поміняно місцями** | **0** | 🔴 **НЕ БУЛО ЗАПІНЕНО** |

#### 🔴 Знахідка: фікс W1 не мав тесту

Порядок «сховище конфліктів ПЕРЕД журналом» на межі батчу — це фікс `d8018b2`
(2026-09-20), що закривав тиху втрату класу **G9**. Повернення рядків у до-фіксовий
порядок не валило **нічого** в усій сюїті.

Сам фікс був правильний, і міркування розписані в коді (таблиця чотирьох станів краху
на `drain.ts:1136`). Не було лише того, що відрізняє «полагоджено» від «не зламається
знову»: **жоден рефактор, який переставив би ці два рядки, не був би помічений.**

Закрито тестом `W1:` у `tests/sync2/drain-conflicts.test.ts`, і він **RED-перевірений** —
з поверненою мутацією падає саме він. Сформульований як ІНВАРІАНТ, а не як порядок
рядків («коли персиститься стан із прапорцем, запис для цього шляху вже має читатися з
диска незалежним читачем»), щоб майбутня перебудова, яка зберігає гарантію іншим
способом, лишалась зеленою.

**Урок, який варто винести ширше:** пара «дефект знайдено → фікс написано → міркування
задокументовано» виглядає як завершена робота, але без проби вона не доводить, що фікс
переживе наступний рефактор. 2026-09-20 той самий метод знайшов завищені клітинки
покриття; тут — фікс без пінця. Обидва рази проба знаходила те, чого перегляд не бачив.

### IX.3.1 Прохід 4 — проби по категоріях §VIII A.1 / C / F / H / I / L (2026-09-23)

Те, що IX.4 нижче називала «наступним кандидатом»: категорії, закриті гуртом
2026-09-20, під тим самим методом. **Двадцять чотири проби** в таблиці, по одній
мутації за раз; двадцять пʼята (розворот порядку обробки батчу) описана окремо під
заголовком про L.1 — вона не в таблиці, бо її нуль падінь означає протилежне до інших
нулів.

| # | категорія | мутація | падінь | вердикт |
|---|---|---|---|---|
| 1 | A.1 | гілку `.obsidian/` знято повністю | 7 | ✅ запінено |
| 2 | A.1 | delete-vs-edit: перемагає ВИДАЛЕННЯ, не жива сторона | 3 | ✅ запінено |
| 3 | A.1 | 3.b.1.a віддає ВІДСУТНЮ сторону | 5 | ✅ запінено |
| 4 | A.1 | mtime-tiebreak без явної null-перевірки (голий `>`) | 1 | ✅ запінено |
| 5 | C | **seeding: наявний tracked-запис НЕ піднімається у конфлікт** | **0** | 🔴 **дірка** → J.3b |
| 6 | C | RECONCILE ніколи не скидає прапорець | 1 | ✅ запінено |
| 7 | C | STEP2-маршрутизація вимкнена | 4 | ✅ запінено |
| 8 | C | **no-op fold усе одно запускає replace-транзакцію** | **0** | 🔴 **дірка** → C.5b |
| 9 | C | **`merged.mtime` від merge, не від згорнутого коміту** | **0** | 🔴 **дірка** → assert у C.3 |
| 10 | F | sweep: знято `referenced.has(sha)` | 6 | ✅ запінено |
| 11 | F | джерело 1 (метадані черги) прибрано | 1 | ✅ запінено |
| 12 | F | **джерело 2 (journal baseSha) прибрано** | **0** | 🔴 **дірка** → S3b |
| 13 | F | **джерело 3 (conflictBase) прибрано** | **0** | 🔴 **дірка** → S3b |
| 14 | F | **джерело 5 (Deleted-bin) прибрано** | **0** | 🔴 **дірка** → S3b |
| 15 | H | знято TOCTOU-перевірку Петерсона | 1 | ✅ запінено |
| 16 | H | FIFO: береться НАЙНОВІШИЙ каталог | 2 | ✅ запінено |
| 17 | H | CRASH_RECOVERY лишає невідновний запис у батчі | 4 | ✅ запінено |
| 18 | H | CRASH_RECOVERY «ремонтує» без звірки sha | 1 | ✅ запінено |
| 19 | H | onload-sweep лишає маркер краху стояти | 8 | ✅ запінено |
| 20 | I | synthetic може обійти tracked | 1 | ✅ запінено |
| 21 | I | правило переходу без «було непорожнім» | 6 | ✅ запінено |
| 22 | I | **2.3 не чіпає synthetic, ідентичний базі** | **0** | 🔴 **дірка** → I.4b |
| 23 | L.2 | `buildEntries` не дедупить шлях | 2 | ✅ запінено |
| 24 | L.2 | `mergeEntries` додає замість заміни | 4 | ✅ запінено |

Сім дірок, усі закриті RED-перевіреними тестами (`c30be54`, `918726e`). Код у всіх
семи випадках був **правильний** — не тримався лише нічим.

#### Що саме коштувала кожна дірка

- **#5 (seeding, else-гілка).** J.3 покриває лише гілку `undefined`. Якщо прапорець не
  піднято, tracked-запис із ВИРІВНЯНИМИ половинами читається як чиста локальна зміна —
  і `_diff3` відправляє одну сторону нерозвʼязаного конфлікту на MAIN. J.5 мутацію
  переживає, бо її dedup блокує push із зовсім іншої причини.
- **#8 (no-op fold).** Транзакція §II.11 — ЄДИНА гілка, що знищує докази. Запуск її на
  згортці, якій нічого замінювати, не дивився ніхто.
- **#9 (імʼя сиблінга).** C.3 виводив очікуване імʼя файлу З збереженого `mtime` —
  тобто міг довести лише самоузгодженість. Тепер assert називає ДЖЕРЕЛО.
- **#12–14 (список джерел sweep).** F.5 пінить half сховища (`sweep()` шанує кожне
  передане джерело), S3 — вижинання. Сам СПИСОК не пінив ніхто, а заголовок S3
  («queue/journal/conflict-referenced blobs survive») виписував вексель, який тіло не
  оплачувало: обидва його блоби мають померти. Ціна найгірша у джерела 5 — його власний
  коментар це й каже: байти корзини не референсить більше ніщо, доки видалення не дійде
  до батчу.
- **#22 (synthetic у 2.3).** I.4 виглядає як покриття, але його synthetic ділить sha з
  tracked-сиблінгом, тож група 2.2 видаляє файл раніше, ніж 2.3 на нього гляне —
  твердження проходило з неправильної причини.

#### L.1 — чесно: не проба

«Послідовна обробка файлів у батчі, пікова памʼять O(1 файл)» — це не рядок, це форма
всієї функції: тіло циклу ~380 рядків, з `return` із зовнішньої функції, `continue` і
скрізною змінною станом між ітераціями (`conflictHeadHash`, `error422Count`,
`restartFromFlush`). Жодна однорядкова мутація не робить його паралельним, а
«пікова памʼять» не спостережна з тесту, який памʼять не міряє. Найближча доступна
проба — **розворот порядку обробки батчу — дала 0 падінь, і це правильно**: у батчі
кожен шлях трапляється рівно раз (L.2), тож порядок і не має значення. L.1 лишається
структурним інваріантом, який тримає рецензія, а не сюїта. L.3 (MAIN ∥ CONFLICT)
запінено через C.1 і пробу #7.

### IX.3.2 Знахідка збоку: замерзлий годинник тримав усю §VIII G (2026-09-23)

Не проба — побічний результат фіксу §II.7.1. Але той самий жанр, тому тут.

`buildConflictBranchName` — чиста функція від `(deviceLabel, nowMs)`. У G-сюїті
`now` заморожений (`1_800_000_000_000`), а її hot-фейк був **write-only**:
`update()` писав у масив, `getConflictBranch()` повертав плоский `null`. Отже гілка
НЕ переносилась між drain'ами взагалі — а тести проходили, бо drain, який «наново
вигадував» ім'я, отримував байт-у-байт той самий рядок і знаходив ту саму гілку.

У продакшені мітка часу рухається щомілісекунди, і переносить гілку від drain'а, що
її народив, до drain'а, що її фіналізує, **тільки hot-пара**. Тобто вся G-серія
пінила механізм, якого у фейку не було, через збіг, якого в реальності не буває.
Виявилось це аж тоді, коли ліниве мінтування прибрало ту саму регенерацію, на якій
збіг їхав: 9 тестів G впали разом.

Фейк тепер читає назад те, що записав `update()` — як робить продакшен. Додатково
той самий шов перевірено на РЕАЛЬНОМУ GitHub (`tests/integration/scenarios/sync2/
conflicts`, 14 тестів) — там годинник справжній.

**Урок у копилку до проходів 3–4:** мутаційна проба питає «чи щось впаде, якщо
зламати код». Вона за побудовою не бачить випадку, де тест зелений через властивість
ГАРНІТУРИ, а не коду. Дешевий запобіжник для наступного разу: фейк, що імітує
сховище, має ЧИТАТИ НАЗАД те, що в нього пишуть — write-only fake не моделює стан, він
його імітує.

### IX.4 Що НЕ перевірено і чим це закривати

- ~~Категорії A.1/C/F/H/I/L мутаційно не перевірялись~~ — **закрито проходом 4
  (IX.3.1, 2026-09-23)**, 24 проби, 7 дірок знайдено й закрито. Лишається L.1, який
  не проба (див. вище).
- **§III (~2540 рядків псевдокоду) не звірявся рядок-у-рядок** — звірялись його
  несучі контракти через §II.x. Повне порядкове звіряння §III — окрема робота.
- **§VIII D.1-D.12, 6 вікон §II.11, K.1** — не покриття, а Фаза 6; поза цим аудитом.
...
> **W1 (додано 2026-09-20) — перша точка цієї категорії, що має тест.** Крах між STEP1
> і епілогом: гілку запушено, прапорець у журналі, `conflicts.json` ще порожній.
> До фіксу давало ТИХУ ВТРАТУ (деталі й таблиця станів — §V «інваріант пари», історія
> діагнозу — §VIII J). Тести: «W1 (§VIII D)» (наскрізний: крах у Vault-step → redo →
> конфлікт живий, sibling відтворено, main незмінний, наступна правка йде `step2-existing`)
> і «W1 ordering, benign half». Решта точок D.1-D.12 + 6 вікон §II.11 лишаються Фазі 6.
...
#### D.21-D.22 — дві НЕПОКРИТІ точки, знайдені аудитом покриття (2026-09-20, тестів нема свідомо)

**D.21. Епілог просуває baseline повз remote-версію, яку fold так і не втягнув.**
Після фіксу E.1 fold-сайт розрізняє два випадки правильно: мережа/токен — аборт усього
drain-у; **підтверджена відсутність** блоба в репо (сервер чесно відповів «нема») — вузький
per-path запис у `vaultStepErrors`, drain іде далі (це і є E.2). Але епілог крок 1 переносить
baseline для кожного tracked-шляху з не-null `remote.sha` — а він тут не-null. Наслідок,
**виведений міркуванням, НЕ відтворений у харнесі** (на відміну від W1):

```
baseline і hot-якір просунуто на R2
 → користувач розв'язує конфлікт проти СТАРОГО sibling-а R1
 → видаляє sibling → запис закривається на переході «непорожній→порожній»
 → комітить зведений вміст
 → diff3: base == remote == R2, local == розв'язання
 → правило 4.4 «чистий push» → R2 на main замінено вмістом, що його не враховував
```

Термінальна форма — та сама, що у W1 (тихе затирання theirs), але тригер практично
недосяжний: блоб згадується ЖИВИМ деревом, тож `getBlobFromRepo` мусить повернути `null`
саме по ньому — потрібна гонка з force-push плюс GC («клас пошкодження репозиторію», уже
названий у коментарі коду). Рішення власника 2026-09-20: **лишити анотацією, не пінити**.
Форма фіксу, коли дійде черга: не просувати baseline для шляхів, що записали
`vaultStepError` (тягне за собою решту епілогу — тому Фаза 6, а не принагідно).

**D.22. Третя точка sweep (`sync_store`) на onload не існує.**
§VIII F.7 вимагає трьох точок: старт drain-у, кінець drain-у, **onload плагіна**. У коді є
дві (`drain.ts`, обидві навколо циклу батчів); окремого виклику на onload нема ніде, і сам
`tests/sync2/sync-store.test.ts` позначає F.7 як «wiring». Де-факто третя точка працює лише
через стартовий drain — тобто **не працює у користувача з вимкненим «Sync on startup»**:
осиротілі після краху блоби пролежать до наступного ручного синку. Наслідок — місце на
диску, не коректність (сховище content-addressed, осиротілий блоб інертний). Фікс — один
виклик у `main.ts` onload з тими самими ін'єктованими джерелами.
...
1. Усі 5 Vault-step NETWORK_ERROR-сайтів абортують ВЕСЬ drain (той самий шлях, що й TOKEN_EXPIRED) — параметризований
   тест на кожен сайт.
   ✅ **ЗАКРИТО 2026-09-20 — і знайшло ДЕФЕКТ.** Сайтів у чинному коді **дев'ять**, не п'ять
   (спека писалась до імплементації); вісім поверталися `statusFromError(...)`, а один — ні:
   матеріалізація байтів результату fold-у (`drain.ts`, гілка `merged.blob === null` у
   Case 2 Vault-step-у) згортала `r.error` у `null` і падала у вузький per-path запис
   `vaultStepErrors`. Наслідок: мертва мережа **і TOKEN_EXPIRED** (його `retry` повертає
   БЕЗ ретраю, п.7) тихо пропускали fold, drain звітував `ok`, а епілог просував baseline
   повз ту remote-версію, що так і не дійшла до sibling-а. Тести — у
   `tests/sync2/drain-conflicts.test.ts` («E.1 (plain pull site)» / «E.1 (fold-result site)»,
   по два класи помилок на сайт); RED-верифіковано ДО фіксу (обидва fold-кейси давали `ok`).
   ⚠️ Досяжність цієї гілки у здоровому потоці вузька — потрібне `conflictBase == sibling`
   (правило 4.3 у fold-і), що природно дає хіба відновлення з durable-сховища після краху;
   тест сіє цей стан явно, бо контракт аборту від способу потрапляння не залежить.
2. NOT_FOUND-сайти (підтверджено відсутні дані, не мережева помилка) лишаються вузьким skip/cancel, НЕ абортом.
   ✅ **ЗАКРИТО 2026-09-20.** Конфліктні NOT_FOUND-сайти вже тримали C.8 (нуль sibling-ів →
   cancel) і C.9 (є sibling → skip); додано неконфліктний Vault-step-сайт («E.2: a CONFIRMED
   absence…»): `result == null` БЕЗ помилки — це per-path запис, решта drain-у доходить до
   кінця. Разом із п.1 це і є та межа, заради якої фікс п.1 розрізняє `error` і `null`:
   переплутати їх означало б, що один зіпсований шлях вічно блокує весь синк.
...
> ✅ **Категорія закрита 2026-09-20.** Додані G.1, G.4, G.5, G.14; G.2 покритий тестом C.1
> (анотовано там, probe-верифіковано); G.11 — остання ассерція G.9-тесту.
>
> ⚠️ **Урок по дорозі: C.2 НЕ покриває G.1**, хоч і виглядає як він. Повторний коміт того
> самого вмісту взагалі не доходить до `shouldPushToConflictBranch`: відновлений
> плейсхолдер робить `tracked.base == local`, і запис відсіюється як «без змін» ще до
> STEP2 — вимкнення гілки «журнал підтверджує» лишає C.2 зеленим. Тому в G.1 `tracked.base`
> навмисно РОЗВОДИТЬСЯ з local через журнал незавершеного прогону, і лише тоді STEP2
> справді виконується. Рішення «is skipped» захищене ДВІЧІ (порівняння
> `conflictBase.sha` у викликача STEP2 + те саме всередині хелпера для ДРУГОГО викликача,
> STEP1-crash-restart), тож probe, який озброює ассерцію, мусить зняти обидва.
...
> ✅ **Категорія закрита 2026-09-20.** J.3/J.4/J.5/J.7 були; додано J.1 і J.6, а J.2 виявився
> уже покритим тестом G.6 (там і анотовано). Деталі покриття — у назвах тестів
> `tests/sync2/drain-conflicts.test.ts`.
>
> ✅ **ЗНАЙДЕНЕ ТУТ ВІКНО КРАХУ ВИПРАВЛЕНО 2026-09-20** — фікс = інваріант пари в §V
> (`conflictStore.save` перед `journal.persist` на межі батчу), регресія = тест
> «W1 (§VIII D)» у `tests/sync2/drain-conflicts.test.ts` (RED-верифіковано до фіксу:
> запис зникав, гілка гинула, LOCAL лягав на main) + «W1 ordering, benign half»
> (доводить, що єдиний досяжний проміжний стан нешкідливий). Опис вікна нижче
> лишається як історія діагнозу.
>
> ⚠️ **ЗНАХІДКА при написанні J.1 — вікно краху STEP1 → епілог (виправлено, див. вище).**
> Durable-запис конфлікту з'являється лише в епілог-кроці 2, а STEP1 до того тримає його
> в журналі. Якщо drain помирає МІЖ ними (гілку вже запушено, sibling уже на диску,
> `conflicts.json` ще порожній), наступний drain читає журнал, бачить `isManualConflict=true`
> і порожній авторитетний скан — і **RECONCILE (J.6) чесно гасить прапорець**, бо його
> правило каже «нема в скані ⇒ розв'язано ззовні». Наслідок: tracked-конфлікт тихо
> деградує в orphan-sibling (тобто в synthetic), а FINALIZE, побачивши нуль конфліктів,
> зливає й ВИДАЛЯЄ conflict-гілку. Дані не гинуть (local у vault, theirs у sibling-файлі,
> main незмінний — merge іде деревом main), але облік втрачається.
> Відтворено в юніт-харнесі 2026-09-20; J.1 навмисно перевіряє САМЕ відновлення
> (ім'я гілки + tracked-стан), а не кінцевий стан, щоб не пінити цю поведінку як правильну.
> Розглядались два кандидати; **обраний — durable-save раніше**, і межа виявилась вужчою,
> ніж «на STEP1/STEP3»: достатньо ОДНОГО збереження перед per-batch `journal.persist`
> (§V, інваріант пари), бо тільки там прапорець стає довговічним.
> Двогілковий пошук synthetic (§4.3.1 DOT-FILES, Крок C) цю проблему **не закриває** —
> у формі W1 sibling-файлу на диску нема взагалі, шукати нема чого; він лишається в черзі
> заради панелі й dot-простору. Регресія живе в §VIII D.
