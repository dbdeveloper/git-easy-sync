...
> ✅ **Категорія закрита 2026-09-20.** G.4 (жива перевірка: інший sha АБО шляху нема на гілці
> → push) і G.5 (ім'я гілки в журналі ДО першого мережевого дотику — перевіряється ПОРЯДКОМ
> подій, не фактом) додані; G.1/G.2 були покриті тестами C.2/C.1 і тепер там анотовані;
> G.11 — остання ассерція G.9-тесту. G.14 — чотири значення `compare.status` окремими
> кейсами. ⚠️ G.1 покрито В ПОЛОВИНІ: C.2 доводить «push не відбувся», але не строге
> «і без мережевого виклику».
