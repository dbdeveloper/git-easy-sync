...
Без слів pull/push: для людини, що не знає git, вони порожні. Технічно це ОДИН
`Notice(msg, 0)`, що живе весь синк і оновлюється через `setMessage()`; наприкінці
ховається й замінюється підсумком (`Sync done — 6 sent, 2 received`) — так повертається
зникле при THE SWITCH завершальне повідомлення.
