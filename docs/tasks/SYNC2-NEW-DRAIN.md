...
Розвʼязок намацаний у тому ж SPIKE §4 і лежить незробленим: **обхід дерева по
каталогах** — один `GET /git/trees/{sha}` на рівень, без 7 МБ-стелі. Це зняло б обрив
одразу для обох шарів. Доти попередження про обрізання друкує `perPathRequests`, щоб
ціна була видима в лозі, а не виглядала як зависання.
