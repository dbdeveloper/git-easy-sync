...
| **Vault-step** | межа файлу | 🔴 **не було — додано 2026-09-26** |
...
`onSyncCompleted` тому несе ДВА прапорці, і різниця між ними змістовна:

- `ok: false` — не звітувати про успіх (помилка АБО скасування);
- `cancelled: true` — це було рішення користувача, і воно заслуговує підтвердження:
  **«Sync canceled»**.

Помилка свого підсумку не отримує: у неї вже є власне повідомлення, і два тости про
одну подію — шум. А от «requested» у момент кліку і «canceled» по факту зупинки — це
**різні моменти**: drain може зупинятись не миттєво, і користувач має знати, що він
справді став.
