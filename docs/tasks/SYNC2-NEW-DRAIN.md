    ...
    while true:                                                            
        if restart_batch:
            ...
            if head_hash == null:
                ...
                # односпрямована (запушити локальне). Єдина реальна вимога цього випадку
                # лежить НЕ тут, а в клієнті: `pushCommit`/`updateReference` мусять уміти
                # створити ПЕРШИЙ коміт на неіснуючій гілці (старий двигун має це як
                # "Case 1 bare repo" — саме той шматок і запозичуємо).
                remote_files = []
